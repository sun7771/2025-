...
const lv_obj_class_t lv_canvas_class = {
		.constructor_cb = lv_canvas_constructor, .destructor_cb =
				lv_canvas_destructor, .instance_size = sizeof(lv_canvas_t),
...
void lv_canvas_transform(lv_obj_t *obj, lv_img_dsc_t *src_img, int16_t angle,
	...
	for (y = 0; y < dest_img->header.h; y++) {
		if (y + offset_y >= 0) {
			lv_draw_sw_transform(NULL, &dest_area, src_img->data,
					src_img->header.w, src_img->header.h, src_img->header.w,
			...
			for (x = 0; x < dest_img->header.w; x++) {
				...
				if (x + offset_x >= dest_img->header.w)
					break;
				if (abuf[x]) {
					lv_img_buf_set_px_color(dest_img, x + offset_x,
							y + offset_y, cbuf[x]);
					lv_img_buf_set_px_alpha(dest_img, x + offset_x,
							y + offset_y, abuf[x]);
				}
			}
...
void lv_canvas_blur_hor(lv_obj_t *obj, const lv_area_t *area, uint16_t r) {
	...
	lv_coord_t line_w = lv_img_buf_get_img_size(canvas->dsc.header.w, 1,
	...
	for (y = a.y1; y <= a.y2; y++) {
		...
		for (x = a.x1 - r_back; x <= a.x1 + r_front; x++) {
			x_safe = x < 0 ? 0 : x;
			x_safe =
					x_safe > canvas->dsc.header.w - 1 ?
							canvas->dsc.header.w - 1 : x_safe;
...
void lv_canvas_blur_ver(lv_obj_t *obj, const lv_area_t *area, uint16_t r) {
	...
	bool has_alpha = lv_img_cf_has_alpha(canvas->dsc.header.cf);
	lv_coord_t col_w = lv_img_buf_get_img_size(1, canvas->dsc.header.h,
			canvas->dsc.header.cf);
	...
	for (x = a.x1; x <= a.x2; x++) {
		...
		for (y = a.y1 - r_back; y <= a.y1 + r_front; y++) {
			y_safe = y < 0 ? 0 : y;
			y_safe =
					y_safe > canvas->dsc.header.h - 1 ?
							canvas->dsc.header.h - 1 : y_safe;
...
void lv_canvas_draw_rect(lv_obj_t *canvas, lv_coord_t x, lv_coord_t y,
		lv_coord_t w, lv_coord_t h, const lv_draw_rect_dsc_t *draw_dsc) {
	LV_ASSERT_OBJ(canvas, MY_CLASS);
...
void lv_canvas_draw_text(lv_obj_t *canvas, lv_coord_t x, lv_coord_t y,
		lv_coord_t max_w, lv_draw_label_dsc_t *draw_dsc, const char *txt) {
	LV_ASSERT_OBJ(canvas, MY_CLASS);
...
void lv_canvas_draw_polygon(lv_obj_t *canvas, const lv_point_t points[],
		uint32_t point_cnt, const lv_draw_rect_dsc_t *draw_dsc) {
	LV_ASSERT_OBJ(canvas, MY_CLASS);
