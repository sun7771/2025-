...
const lv_obj_class_t lv_arc_class = { .constructor_cb = lv_arc_constructor,
		.event_cb = lv_arc_event, .instance_size = sizeof(lv_arc_t), .editable =
				LV_OBJ_CLASS_EDITABLE_TRUE, .base_class = &lv_obj_class };
...
static void lv_arc_event(const lv_obj_class_t *class_p, lv_event_t *e) {
	...
	if (code == LV_EVENT_PRESSING) {
		...
		if (arc->dragging == false) {
			lv_coord_t indic_width = lv_obj_get_style_arc_width(obj,
					LV_PART_INDICATOR);
			r -= indic_width;
...
static void lv_arc_draw(lv_event_t *e) {
	...
	lv_coord_t bottom_indic = lv_obj_get_style_pad_bottom(obj,
			LV_PART_INDICATOR);
	lv_coord_t indic_r = arc_r
...
static lv_coord_t get_angle(const lv_obj_t *obj) {
	...
	if (arc->type == LV_ARC_MODE_NORMAL) {
		...
		int32_t range_midpoint = (int32_t) (arc->min_value + arc->max_value)
				/ 2;
