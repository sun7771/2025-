...
const lv_obj_class_t lv_slider_class = {
		.constructor_cb = lv_slider_constructor, .event_cb = lv_slider_event,
		.editable = LV_OBJ_CLASS_EDITABLE_TRUE, .group_def =
				LV_OBJ_CLASS_GROUP_DEF_TRUE, .instance_size =
				sizeof(lv_slider_t), .base_class = &lv_bar_class };
...
static void lv_slider_event(const lv_obj_class_t *class_p, lv_event_t *e) {
	...
	if (code == LV_EVENT_HIT_TEST) {
		...
		if (type == LV_SLIDER_MODE_NORMAL
			...
			lv_base_dir_t base_dir = lv_obj_get_style_base_dir(obj,
					LV_PART_MAIN);
					...
					dist_left = LV_ABS(
							(slider->left_knob_area.x1
									+ (slider->left_knob_area.x2
											- slider->left_knob_area.x1) / 2)
									- p.x);
					dist_right = LV_ABS(
							(slider->right_knob_area.x1
									+ (slider->right_knob_area.x2
											- slider->right_knob_area.x1) / 2)
									- p.x);
					...
					dist_left = LV_ABS(
							(slider->left_knob_area.y1
									+ (slider->left_knob_area.y2
											- slider->left_knob_area.y1) / 2)
									- p.y);
					dist_right = LV_ABS(
							(slider->right_knob_area.y1
									+ (slider->right_knob_area.y2
											- slider->right_knob_area.y1) / 2)
									- p.y);
		...
		if (is_slider_horizontal(obj)) {
			const lv_coord_t bg_left = lv_obj_get_style_pad_left(obj,
					LV_PART_MAIN);
			const lv_coord_t bg_right = lv_obj_get_style_pad_right(obj,
					LV_PART_MAIN);
			...
			const lv_coord_t bg_top = lv_obj_get_style_pad_top(obj,
					LV_PART_MAIN);
			const lv_coord_t bg_bottom = lv_obj_get_style_pad_bottom(obj,
					LV_PART_MAIN);
		...
		lv_coord_t trans_w = lv_obj_get_style_transform_width(obj,
				LV_PART_KNOB);
		lv_coord_t trans_h = lv_obj_get_style_transform_height(obj,
				LV_PART_KNOB);
		...
		*s = LV_MAX(*s, knob_size);
		...
		if (c == LV_KEY_RIGHT || c == LV_KEY_UP) {
			if (!slider->left_knob_focus)
				lv_slider_set_value(obj, lv_slider_get_value(obj) + 1,
						LV_ANIM_ON);
			else
...
static void draw_knob(lv_event_t *e) {
	...
	if (lv_slider_get_mode(obj) != LV_SLIDER_MODE_RANGE) {
		...
		lv_memcpy(&knob_rect_dsc_tmp, &knob_rect_dsc,
		...
		if (is_horizontal) {
			...
			knob_area.x1 = LV_SLIDER_KNOB_COORD(!is_rtl,
		...
		lv_memcpy(&knob_rect_dsc, &knob_rect_dsc_tmp,
				sizeof(lv_draw_rect_dsc_t));
