...
static lv_coord_t get_row_height(lv_obj_t *obj, uint16_t row_id,
		const lv_font_t *font, lv_coord_t letter_space, lv_coord_t line_space,
 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_table_class = { .constructor_cb = lv_table_constructor,
		.destructor_cb = lv_table_destructor, .event_cb = lv_table_event,
		.width_def = LV_SIZE_CONTENT, .height_def = LV_SIZE_CONTENT,
		.base_class = &lv_obj_class, .editable = LV_OBJ_CLASS_EDITABLE_TRUE,
		.group_def = LV_OBJ_CLASS_GROUP_DEF_TRUE, .instance_size =
				sizeof(lv_table_t), };
/**********************
 *      MACROS
 **********************/
...
void lv_table_set_row_cnt(lv_obj_t *obj, uint16_t row_cnt) {
	...
	table->cell_data = lv_mem_realloc(table->cell_data,
			table->row_cnt * table->col_cnt * sizeof(char*));
...
void lv_table_set_col_cnt(lv_obj_t *obj, uint16_t col_cnt) {
	...
	char **new_cell_data = lv_mem_alloc(
			table->row_cnt * table->col_cnt * sizeof(char*));
	...
	table->col_w = lv_mem_realloc(table->col_w,
			col_cnt * sizeof(table->col_w[0]));
	...
	for (row = 0; row < table->row_cnt; row++) {
		...
		for (col = 0; col < table->col_cnt; col++) {
			...
			if ((rect_dsc_def.border_side & LV_BORDER_SIDE_RIGHT)
				...
				cell_area_border.x2 += rect_dsc_def.border_width / 2
			...
			if ((rect_dsc_def.border_side & LV_BORDER_SIDE_BOTTOM)
				...
				cell_area_border.y2 += rect_dsc_def.border_width / 2
						+ (rect_dsc_def.border_width & 0x1);
			}
			...
			if (cell_state == LV_STATE_DEFAULT) {
				lv_memcpy(&rect_dsc_act, &rect_dsc_def,
						sizeof(lv_draw_rect_dsc_t));
				lv_memcpy(&label_dsc_act, &label_dsc_def,
			...
			if (table->cell_data[cell]) {
				const lv_coord_t cell_left = lv_obj_get_style_pad_left(obj,
						LV_PART_ITEMS);
				const lv_coord_t cell_right = lv_obj_get_style_pad_right(obj,
						LV_PART_ITEMS);
				const lv_coord_t cell_top = lv_obj_get_style_pad_top(obj,
						LV_PART_ITEMS);
				const lv_coord_t cell_bottom = lv_obj_get_style_pad_bottom(obj,
						LV_PART_ITEMS);
				...
				bool label_mask_ok;
				label_mask_ok = _lv_area_intersect(&label_clip_area, &clip_area,
						&cell_area);
				if (label_mask_ok) {
...
static void refr_size_form_row(lv_obj_t *obj, uint32_t start_row) {
	const lv_coord_t cell_pad_left = lv_obj_get_style_pad_left(obj,
			LV_PART_ITEMS);
	const lv_coord_t cell_pad_right = lv_obj_get_style_pad_right(obj,
			LV_PART_ITEMS);
	const lv_coord_t cell_pad_top = lv_obj_get_style_pad_top(obj,
			LV_PART_ITEMS);
	const lv_coord_t cell_pad_bottom = lv_obj_get_style_pad_bottom(obj,
			LV_PART_ITEMS);

	lv_coord_t letter_space = lv_obj_get_style_text_letter_space(obj,
			LV_PART_ITEMS);
	...
	for (i = start_row; i < table->row_cnt; i++) {
		lv_coord_t calculated_height = get_row_height(obj, i, font,
				letter_space, line_space, cell_pad_left, cell_pad_right,
...
static void refr_cell_size(lv_obj_t *obj, uint32_t row, uint32_t col) {
	...
	lv_table_t *table = (lv_table_t*) obj;
	lv_coord_t calculated_height = get_row_height(obj, row, font, letter_space,
			line_space, cell_pad_left, cell_pad_right, cell_pad_top,
			cell_pad_bottom);
...
static lv_res_t get_pressed_cell(lv_obj_t *obj, uint16_t *row, uint16_t *col) {
	...
	if (col) {
		...
		if (lv_obj_get_style_base_dir(obj, LV_PART_MAIN) == LV_BASE_DIR_RTL) {
			x = obj->coords.x2 - lv_obj_get_style_pad_right(obj, LV_PART_MAIN)
					- x;
		} else {
			x -= obj->coords.x1;
