...
const lv_obj_class_t lv_img_class = { .constructor_cb = lv_img_constructor,
		.destructor_cb = lv_img_destructor, .event_cb = lv_img_event,
		.width_def = LV_SIZE_CONTENT, .height_def = LV_SIZE_CONTENT,
		.instance_size = sizeof(lv_img_t), .base_class = &lv_obj_class };
...
void lv_img_set_src(lv_obj_t *obj, const void *src) {
	...
	if (src_type == LV_IMG_SRC_SYMBOL) {
		...
		lv_coord_t line_space = lv_obj_get_style_text_line_space(obj,
				LV_PART_MAIN);
		lv_point_t size;
		lv_txt_get_size(&size, src, font, letter_space, line_space,
		...
		static void lv_img_event(const lv_obj_class_t *class_p, lv_event_t *e) {
			...
			if (code == LV_EVENT_STYLE_CHANGED) {
				...
				if (img->angle || img->zoom != LV_IMG_ZOOM_NONE) {
					...
					lv_coord_t w = lv_obj_get_width(obj);
					lv_coord_t h = lv_obj_get_height(obj);
					_lv_img_buf_get_transformed_area(&a, w, h, img->angle,
							img->zoom, &img->pivot);
					*s = LV_MAX(*s, -a.x1);
					*s = LV_MAX(*s, -a.y1);
					*s = LV_MAX(*s, a.x2 - w);
				...
				if (img->w == lv_obj_get_width(obj)
					...
					lv_coord_t h = lv_obj_get_height(obj);
					lv_area_t coords;
					_lv_img_buf_get_transformed_area(&coords, w, h, img->angle,
							img->zoom, &img->pivot);
			...
			if (code == LV_EVENT_COVER_CHECK) {
				...
				if (img->zoom == LV_IMG_ZOOM_NONE) {
					...
					_lv_img_buf_get_transformed_area(&a, lv_obj_get_width(obj),
							lv_obj_get_height(obj), 0, img->zoom, &img->pivot);
					a.x1 += obj->coords.x1;
					a.y1 += obj->coords.y1;
					a.x2 += obj->coords.x1;
					a.y2 += obj->coords.y1;
				...
				lv_coord_t pleft = lv_obj_get_style_pad_left(obj, LV_PART_MAIN)
						+ border_width;
				lv_coord_t pright = lv_obj_get_style_pad_right(obj,
						LV_PART_MAIN) + border_width;
				lv_coord_t ptop = lv_obj_get_style_pad_top(obj, LV_PART_MAIN)
						+ border_width;
				lv_coord_t pbottom = lv_obj_get_style_pad_bottom(obj,
						LV_PART_MAIN) + border_width;
				...
				if (code == LV_EVENT_DRAW_MAIN) {
					...
					lv_point_t img_size_final = lv_img_get_transformed_size(
							obj);

					if (img->obj_size_mode == LV_IMG_SIZE_MODE_REAL) {
						img_max_area.x1 -= ((img->w - img_size_final.x) + 1)
								/ 2;
						img_max_area.x2 -= ((img->w - img_size_final.x) + 1)
								/ 2;
						img_max_area.y1 -= ((img->h - img_size_final.y) + 1)
								/ 2;
						img_max_area.y2 -= ((img->h - img_size_final.y) + 1)
								/ 2;
					} else {
						img_max_area.x2 = img_max_area.x1
								+ lv_area_get_width(&bg_coords) - 1;
						img_max_area.y2 = img_max_area.y1
					...
					if (img->src_type == LV_IMG_SRC_FILE
						...
						for (; coords_tmp.y1 < img_max_area.y2; coords_tmp.y1 +=
								img_size_final.y, coords_tmp.y2 +=
								img_size_final.y) {
							...
							for (; coords_tmp.x1 < img_max_area.x2;
									coords_tmp.x1 += img_size_final.x, coords_tmp.x2 +=
											img_size_final.x) {
						...
						lv_obj_init_draw_label_dsc(obj, LV_PART_MAIN,
								&label_dsc);
