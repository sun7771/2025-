...
const lv_obj_class_t lv_switch_class = {
		.constructor_cb = lv_switch_constructor, .destructor_cb =
				lv_switch_destructor, .event_cb = lv_switch_event, .width_def =
				(4 * LV_DPI_DEF) / 10, .height_def = (4 * LV_DPI_DEF) / 17,
		.group_def = LV_OBJ_CLASS_GROUP_DEF_TRUE, .instance_size =
				sizeof(lv_switch_t), .base_class = &lv_obj_class };
...
static void lv_switch_event(const lv_obj_class_t *class_p, lv_event_t *e) {
	...
	if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
		...
		lv_coord_t knob_size = LV_MAX4(knob_left, knob_right, knob_bottom,
				knob_top);
...
static void draw_main(lv_event_t *e) {
	...
	if (LV_SWITCH_IS_ANIMATING(sw)) {
		...
		anim_value_x =
				(anim_length * sw->anim_state) / LV_SWITCH_ANIM_STATE_END;
	} else {
...
static void lv_switch_trigger_anim(lv_obj_t *obj) {
	...
	if (anim_dur_full > 0) {
		...
		if (sw->anim_state == LV_SWITCH_ANIM_STATE_INV) {
			anim_start =
					chk ? LV_SWITCH_ANIM_STATE_START : LV_SWITCH_ANIM_STATE_END;
			anim_end =
					chk ? LV_SWITCH_ANIM_STATE_END : LV_SWITCH_ANIM_STATE_START;
		}
