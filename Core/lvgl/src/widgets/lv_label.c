...
const lv_obj_class_t lv_label_class = { .constructor_cb = lv_label_constructor,
		.destructor_cb = lv_label_destructor, .event_cb = lv_label_event,
		.width_def = LV_SIZE_CONTENT, .height_def = LV_SIZE_CONTENT,
		.instance_size = sizeof(lv_label_t), .base_class = &lv_obj_class };
...
void lv_label_set_long_mode(lv_obj_t *obj, lv_label_long_mode_t long_mode) {
	...
	if (label->long_mode
			== LV_LABEL_LONG_DOT&& label->dot_end != LV_LABEL_DOT_END_INV) {
...
void lv_label_get_letter_pos(const lv_obj_t *obj, uint32_t char_id,
	...
	lv_text_align_t align = lv_obj_calculate_style_text_align(obj, LV_PART_MAIN,
			txt);
...
static void draw_main(lv_event_t *e) {
	...
	if (label_draw_dsc.sel_start != LV_DRAW_LABEL_NO_TXT_SEL
		...
		label_draw_dsc.sel_color = lv_obj_get_style_text_color_filtered(obj,
				LV_PART_SELECTED);
		label_draw_dsc.sel_bg_color = lv_obj_get_style_bg_color(obj,
				LV_PART_SELECTED);
	}
	...
	if (label->long_mode
			== LV_LABEL_LONG_SCROLL_CIRCULAR|| lv_area_get_height(&txt_coords) < LV_LABEL_HINT_HEIGHT_LIMIT)
		hint = NULL;
	...
	lv_area_t txt_clip;
	bool is_common = _lv_area_intersect(&txt_clip, &txt_coords,
			draw_ctx->clip_area);
	if (!is_common)
		return;
	...
	if (label->long_mode == LV_LABEL_LONG_SCROLL
		...
		lv_draw_label(draw_ctx, &label_draw_dsc, &txt_coords, label->text,
				hint);
...
static void lv_label_refr_text(lv_obj_t *obj) {
	...
	if (label->long_mode == LV_LABEL_LONG_SCROLL) {
		...
		if (size.x > lv_area_get_width(&txt_coords)) {
			...
			lv_anim_set_time(&a,
					lv_anim_speed_to_time(anim_speed, a.start_value,
							a.end_value));
	...
	/*In roll inf. mode keep the size but start offset animations*/
		...
		const lv_anim_t *anim_template = lv_obj_get_style_anim(obj,
				LV_PART_MAIN);
		uint16_t anim_speed = lv_obj_get_style_anim_speed(obj, LV_PART_MAIN);
		if (anim_speed == 0)
			anim_speed = LV_LABEL_DEF_SCROLL_SPEED;
		...
		if (size.y <= lv_area_get_height(&txt_coords)) { /*No dots are required, the text is short enough*/
			...
			uint32_t byte_id = _lv_txt_encoded_get_byte_id(label->text,
					letter_id);
