...
const lv_obj_class_t lv_roller_class = {
		.constructor_cb = lv_roller_constructor, .event_cb = lv_roller_event,
		.width_def = LV_SIZE_CONTENT, .height_def = LV_DPI_DEF, .instance_size =
				sizeof(lv_roller_t), .editable = LV_OBJ_CLASS_EDITABLE_TRUE,
		.group_def = LV_OBJ_CLASS_GROUP_DEF_TRUE, .base_class = &lv_obj_class };

const lv_obj_class_t lv_roller_label_class = {
		.event_cb = lv_roller_label_event, .instance_size = sizeof(lv_label_t),
...
void lv_roller_set_options(lv_obj_t *obj, const char *options,
	...
	if (mode == LV_ROLLER_MODE_NORMAL) {
		...
		roller->sel_opt_id = ((LV_ROLLER_INF_PAGES / 2) + 0)
				* roller->option_cnt;
...
static void draw_main(lv_event_t *e) {
	...
	/*Post draw when the children are drawn*/
		...
		if (area_ok) {
			...
			int32_t label_y_prop = label->coords.y1
					- (roller_h / 2 + obj->coords.y1); /*label offset from the middle line of the roller*/
			label_y_prop = (label_y_prop * 16384) / lv_obj_get_height(label); /*Proportional position from the middle line (upscaled by << 14)*/

			/*Apply a correction with different line heights*/
			const lv_font_t *normal_label_font = lv_obj_get_style_text_font(obj,
					LV_PART_MAIN);
			lv_coord_t corr = (label_dsc.font->line_height
					- normal_label_font->line_height) / 2;
			...
			lv_coord_t bwidth = lv_obj_get_style_border_width(obj,
					LV_PART_MAIN);
...
static void get_sel_area(lv_obj_t *obj, lv_area_t *sel_area) {
	...
	const lv_font_t *font_sel = lv_obj_get_style_text_font(obj,
			LV_PART_SELECTED);
...
static void refr_position(lv_obj_t *obj, lv_anim_enable_t anim_en) {
	...
	lv_text_align_t align = lv_obj_calculate_style_text_align(label,
			LV_PART_MAIN, lv_label_get_text(label));
...
static lv_res_t release_handler(lv_obj_t *obj) {
	...
	if (lv_indev_get_type(indev) == LV_INDEV_TYPE_POINTER
		...
		if (roller->moved == 0) {
			...
			const lv_font_t *font = lv_obj_get_style_text_font(obj,
					LV_PART_MAIN);
			...
			lv_coord_t mid = obj->coords.y1
					+ (obj->coords.y2 - obj->coords.y1) / 2;
			lv_coord_t label_y1 = label->coords.y1
					+ lv_indev_scroll_throw_predict(indev, LV_DIR_VER);
