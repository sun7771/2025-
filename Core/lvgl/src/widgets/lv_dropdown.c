 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_dropdown_class = { .constructor_cb =
		lv_dropdown_constructor, .destructor_cb = lv_dropdown_destructor,
		.event_cb = lv_dropdown_event, .width_def = LV_DPI_DEF, .height_def =
				LV_SIZE_CONTENT, .instance_size = sizeof(lv_dropdown_t),
		.editable = LV_OBJ_CLASS_EDITABLE_TRUE, .group_def =
				LV_OBJ_CLASS_GROUP_DEF_TRUE, .base_class = &lv_obj_class };

const lv_obj_class_t lv_dropdownlist_class = { .constructor_cb =
		lv_dropdownlist_constructor,
		.destructor_cb = lv_dropdownlist_destructor, .event_cb =
				lv_dropdown_list_event, .instance_size =
				sizeof(lv_dropdown_list_t), .base_class = &lv_obj_class };
...
void lv_dropdown_open(lv_obj_t *dropdown_obj) {
	...
	lv_coord_t border_width = lv_obj_get_style_border_width(dropdown->list,
			LV_PART_MAIN);
	lv_coord_t top = lv_obj_get_style_pad_top(dropdown->list, LV_PART_MAIN)
			+ border_width;
	lv_coord_t bottom = lv_obj_get_style_pad_bottom(dropdown->list,
	...
	if (dir == LV_DIR_BOTTOM)
		lv_obj_align_to(dropdown->list, dropdown_obj, LV_ALIGN_OUT_BOTTOM_LEFT,
				0, 0);
	else if (dir == LV_DIR_TOP)
		lv_obj_align_to(dropdown->list, dropdown_obj, LV_ALIGN_OUT_TOP_LEFT, 0,
				0);
	else if (dir == LV_DIR_LEFT)
		lv_obj_align_to(dropdown->list, dropdown_obj, LV_ALIGN_OUT_LEFT_TOP, 0,
				0);
	else if (dir == LV_DIR_RIGHT)
		lv_obj_align_to(dropdown->list, dropdown_obj, LV_ALIGN_OUT_RIGHT_TOP, 0,
				0);
	...
	lv_text_align_t align = lv_obj_calculate_style_text_align(label,
			LV_PART_MAIN, dropdown->options);
...
static void draw_main(lv_event_t *e) {
	...
	if (dropdown->symbol) {
		...
		if (symbol_type == LV_IMG_SRC_SYMBOL) {
			...
			img_dsc.angle = lv_obj_get_style_transform_angle(obj,
					LV_PART_INDICATOR);
...
static void draw_box(lv_obj_t *dropdown_obj, lv_draw_ctx_t *draw_ctx,
	...
	const lv_font_t *font = lv_obj_get_style_text_font(list_obj,
			LV_PART_SELECTED);
	lv_coord_t line_space = lv_obj_get_style_text_line_space(list_obj,
			LV_PART_SELECTED);
...
static uint16_t get_id_on_point(lv_obj_t *dropdown_obj, lv_coord_t y) {
	...
	lv_coord_t font_h = lv_font_get_line_height(font);
	lv_coord_t line_space = lv_obj_get_style_text_line_space(label,
			LV_PART_MAIN);
