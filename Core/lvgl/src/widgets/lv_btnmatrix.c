...
const lv_obj_class_t lv_btnmatrix_class = { .constructor_cb =
		lv_btnmatrix_constructor, .destructor_cb = lv_btnmatrix_destructor,
		.event_cb = lv_btnmatrix_event, .width_def = LV_DPI_DEF * 2,
		.height_def = LV_DPI_DEF, .instance_size = sizeof(lv_btnmatrix_t),
...
void lv_btnmatrix_set_map(lv_obj_t *obj, const char *map[]) {
	...
	for (row = 0; row < btnm->row_cnt; row++) {
		...
		lv_coord_t row_y1 = ptop + (max_h_no_gap * row) / btnm->row_cnt
				+ row * prow;
		lv_coord_t row_y2 = ptop + (max_h_no_gap * (row + 1)) / btnm->row_cnt
				+ row * prow - 1;
		...
		for (btn = 0; btn < btn_cnt; btn++, btn_tot_i++, txt_tot_i++) {
			...
			lv_coord_t btn_x1 = (max_w_no_gap * row_unit_cnt) / unit_cnt
					+ btn * pcol;
			lv_coord_t btn_x2 = (max_w_no_gap * (row_unit_cnt + btn_u))
					/ unit_cnt + btn * pcol - 1;
...
static void lv_btnmatrix_event(const lv_obj_class_t *class_p, lv_event_t *e) {
	...
	if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
		if (has_popovers_in_top_row(obj)) {
			...
			lv_coord_t s =
					btnm->row_cnt > 0 ?
							lv_obj_get_content_height(obj) / btnm->row_cnt : 0;
	...
	if (code == LV_EVENT_STYLE_CHANGED) {
		...
		if (btnm->btn_id_sel != LV_BTNMATRIX_BTN_NONE) {
			if (button_is_click_trig(btnm->ctrl_bits[btnm->btn_id_sel]) == false
					&& button_is_popover(
							btnm->ctrl_bits[btnm->btn_id_sel]) == false &&
		...
		if (btn_pr != btnm->btn_id_sel) {
			...
			if (btn_pr != LV_BTNMATRIX_BTN_NONE
				...
				if (button_is_click_trig(btnm->ctrl_bits[btn_pr]) == false
						&& button_is_popover(btnm->ctrl_bits[btnm->btn_id_sel])
								== false) {
					uint32_t b = btn_pr;
					res = lv_event_send(obj, LV_EVENT_VALUE_CHANGED, &b);
					if (res != LV_RES_OK)
		...
		if (c == LV_KEY_RIGHT) {
			...
			if (btnm->btn_id_sel == LV_BTNMATRIX_BTN_NONE) {
				...
				lv_coord_t pr_center = btnm->button_areas[btnm->btn_id_sel].x1
						+ (lv_area_get_width(
								&btnm->button_areas[btnm->btn_id_sel]) >> 1);
			...
			if (btnm->btn_id_sel == LV_BTNMATRIX_BTN_NONE) {
				...
				for (area_above = btnm->btn_id_sel; area_above >= 0;
						area_above--) {
