...
int lv_tlsf_check(lv_tlsf_t tlsf) {
	...
	for (i = 0; i < FL_INDEX_COUNT; ++i) {
		for (j = 0; j < SL_INDEX_COUNT; ++j) {
			...
			tlsf_insist(
					block != &control->block_null && "block should not be null");
			...
			while (block != &control->block_null) {
				...
				tlsf_insist(
						block_size(block) >= block_size_min
...
lv_tlsf_t lv_tlsf_create_with_pool(void *mem, size_t bytes) {
	...
	lv_tlsf_add_pool(tlsf, (char*) mem + lv_tlsf_size(),
			bytes - lv_tlsf_size());
	return tlsf;
}
...
void* lv_tlsf_memalign(lv_tlsf_t tlsf, size_t align, size_t size) {
	...
	const size_t size_with_gap = adjust_request_size(
			adjust + align + gap_minimum, align);
