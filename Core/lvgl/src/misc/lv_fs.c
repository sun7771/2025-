...
static lv_fs_res_t lv_fs_read_cached(lv_fs_file_t *file_p, char *buf,
	...
	if (start <= file_position && file_position < end) {
		...
		uint32_t buffer_remaining_length = LV_MIN(
				(uint32_t )buffer_size - buffer_offset,
		...
		if (btr <= buffer_remaining_length) {
			...
			*br = btr;
			...
			if (btr > buffer_size) {
				...
				res = file_p->drv->read_cb(file_p->drv, file_p->file_d,
						(void*) buffer, buffer_size, &bytes_read_to_buffer);
				file_p->cache->start = file_p->cache->end;
				file_p->cache->end = file_p->cache->start
						+ bytes_read_to_buffer;

				uint16_t data_chunk_remaining = LV_MIN(
						btr - buffer_remaining_length, bytes_read_to_buffer);
				lv_memcpy(buf + buffer_remaining_length, buffer,
		...
		if (btr > buffer_size) {
			...
			res = file_p->drv->read_cb(file_p->drv, file_p->file_d, (void*) buf,
					btr, br);
		} else {
...
lv_fs_res_t lv_fs_read(lv_fs_file_t *file_p, void *buf, uint32_t btr,
	...
	if (file_p->drv->cache_size) {
		...
		res = file_p->drv->read_cb(file_p->drv, file_p->file_d, buf, btr,
				&br_tmp);
	}
	...
	if (file_p->drv->cache_size) {
		...
		case LV_FS_SEEK_SET: {
			...
			if (file_p->cache->file_position < file_p->cache->start
				...
				res = file_p->drv->seek_cb(file_p->drv, file_p->file_d,
		...
		case LV_FS_SEEK_END: {
			...
			res = file_p->drv->seek_cb(file_p->drv, file_p->file_d, pos,
					whence);
			if (res == LV_FS_RES_OK) {
				uint32_t tmp_position;
				res = file_p->drv->tell_cb(file_p->drv, file_p->file_d,
						&tmp_position);
