...
typedef lv_color_t (*lv_color_filter_cb_t)(const struct _lv_color_filter_dsc_t*,
		lv_color_t, lv_opa_t);
 ...
 */
LV_ATTRIBUTE_FAST_MEM static inline lv_color_t lv_color_mix(lv_color_t c1,
	...
	LV_COLOR_SET_R(ret,
			LV_UDIV255((uint16_t)LV_COLOR_GET_R(c1) * mix + LV_COLOR_GET_R(c2) * (255 - mix) + LV_COLOR_MIX_ROUND_OFS));
	LV_COLOR_SET_G(ret,
			LV_UDIV255((uint16_t)LV_COLOR_GET_G(c1) * mix + LV_COLOR_GET_G(c2) * (255 - mix) + LV_COLOR_MIX_ROUND_OFS));
	LV_COLOR_SET_B(ret,
			LV_UDIV255((uint16_t)LV_COLOR_GET_B(c1) * mix + LV_COLOR_GET_B(c2) * (255 - mix) + LV_COLOR_MIX_ROUND_OFS));
...
LV_ATTRIBUTE_FAST_MEM static inline void lv_color_premult(lv_color_t c,
		uint8_t mix, uint16_t *out) {
#if LV_COLOR_DEPTH != 1
 ...
 */
LV_ATTRIBUTE_FAST_MEM static inline lv_color_t lv_color_mix_premult(
		uint16_t *premult_c1, lv_color_t c2, uint8_t mix) {
	lv_color_t ret;
#if LV_COLOR_DEPTH != 1
 ...
 */
LV_ATTRIBUTE_FAST_MEM static inline void lv_color_mix_with_alpha(
	...
	/*Both colors have alpha. Expensive calculation need to be applied*/
		...
		if (fg_opa != fg_opa_save || bg_opa != bg_opa_save
			...
			res_opa_saved = 255
					- ((uint16_t) ((uint16_t) (255 - fg_opa) * (255 - bg_opa))
							>> 8);
