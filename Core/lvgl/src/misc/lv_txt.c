...
void lv_txt_get_size(lv_point_t *size_res, const char *text,
	...
	while (text[line_start] != '\0') {
		new_line_start += _lv_txt_get_next_line(&text[line_start], font,
				letter_space, max_width, NULL, flag);

		if ((unsigned long) size_res->y + (unsigned long) letter_height
				+ (unsigned long) line_space > LV_MAX_OF(lv_coord_t)) {
			LV_LOG_WARN(
		...
		lv_coord_t act_line_length = lv_txt_get_width(&text[line_start],
				new_line_start - line_start, font, letter_space, flag);
...
static uint32_t lv_txt_get_next_word(const char *txt, const lv_font_t *font,
		...
		uint32_t *word_w_ptr, lv_text_cmd_state_t *cmd_state, bool force) {
	if (txt == NULL || txt[0] == '\0')
		return 0;
	if (font == NULL)
		return 0;
...
uint32_t _lv_txt_get_next_line(const char *txt, const lv_font_t *font,
		lv_coord_t letter_space, lv_coord_t max_width, lv_coord_t *used_width,
		lv_text_flag_t flag) {
	if (used_width)
		*used_width = 0;
...
lv_coord_t lv_txt_get_width(const char *txt, uint32_t length,
		const lv_font_t *font, lv_coord_t letter_space, lv_text_flag_t flag) {
	if (txt == NULL)
		return 0;
	if (font == NULL)
		return 0;
