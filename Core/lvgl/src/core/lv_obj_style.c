...
void lv_obj_refresh_style(lv_obj_t *obj, lv_style_selector_t selector,
	...
	bool is_layout_refr = lv_style_prop_has_flag(prop,
...
void lv_obj_set_local_style_prop_meta(lv_obj_t *obj, lv_style_prop_t prop,
		uint16_t meta, lv_style_selector_t selector) {
	lv_style_t *style = get_local_style(obj, selector);
...
void _lv_obj_style_create_transition(lv_obj_t *obj, lv_part_t part,
		lv_state_t prev_state, lv_state_t new_state,
...
lv_style_value_t _lv_obj_style_apply_color_filter(const lv_obj_t *obj,
		uint32_t part, lv_style_value_t v) {
	if (obj == NULL)
		return v;
	const lv_color_filter_dsc_t *f = lv_obj_get_style_color_filter_dsc(obj,
			part);
	if (f && f->filter_cb) {
...
_lv_style_state_cmp_t _lv_obj_style_state_compare(lv_obj_t *obj,
		lv_state_t state1, lv_state_t state2) {
	...
	for (i = 0; i < obj->style_cnt; i++) {
		...
		case LV_STYLE_IMG_RECOLOR:
			if (v <= 0)
				...
				value_final.color = lv_color_mix(tr->end_value.color,
						tr->start_value.color, v);
			break;
		...
		for (i = 0; i < obj->style_cnt; i++) {
			if (obj->styles[i].is_trans
				...
				if (lv_style_is_empty(obj->styles[i].style)) {
					lv_obj_remove_style(obj, obj_style->style,
							obj_style->selector);
