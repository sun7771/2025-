...
static void scroll_area_into_view(const lv_area_t *area, lv_obj_t *child,
		lv_point_t *scroll_value, lv_anim_enable_t anim_en);
...
void lv_obj_get_scrollbar_area(lv_obj_t *obj, lv_area_t *hor_area,
	...
	bool rtl =
			lv_obj_get_style_base_dir(obj, LV_PART_SCROLLBAR)
					== LV_BASE_DIR_RTL ? true : false;

	lv_coord_t top_space = lv_obj_get_style_pad_top(obj, LV_PART_SCROLLBAR);
	lv_coord_t bottom_space = lv_obj_get_style_pad_bottom(obj,
			LV_PART_SCROLLBAR);
	...
	if (ver_draw && content_h) {
		...
		lv_coord_t sb_h = ((obj_h - top_space - bottom_space - hor_req_space)
				* obj_h) / content_h;
		sb_h = LV_MAX(sb_h, SCROLLBAR_MIN_SIZE);
		rem = (obj_h - top_space - bottom_space - hor_req_space) - sb_h; /*Remaining size from the scrollbar track that is not the scrollbar itself*/
	...
	if (hor_draw && content_w) {
		...
		lv_coord_t sb_w = ((obj_w - left_space - right_space - ver_reg_space)
				* obj_w) / content_w;
		sb_w = LV_MAX(sb_w, SCROLLBAR_MIN_SIZE);
		rem = (obj_w - left_space - right_space - ver_reg_space) - sb_w; /*Remaining size from the scrollbar track that is not the scrollbar itself*/
		...
		if (scroll_w <= 0) {
			...
			if (rtl) {
				...
				if (hor_area->x1
						< obj->coords.x1 + left_space + ver_reg_space) {
				...
				if (hor_area->x2
						> obj->coords.x2 - ver_reg_space - right_space) {
...
static void scroll_area_into_view(const lv_area_t *area, lv_obj_t *child,
	...
	lv_coord_t pbottom = lv_obj_get_style_pad_bottom(parent, LV_PART_MAIN)
			+ border_width;
	lv_coord_t top_diff = parent->coords.y1 + ptop - area_tmp->y1
			- scroll_value->y;
	lv_coord_t bottom_diff = -(parent->coords.y2 - pbottom - area_tmp->y2
			- scroll_value->y);
	...
	lv_coord_t pright = lv_obj_get_style_pad_right(parent, LV_PART_MAIN)
			+ border_width;
	lv_coord_t left_diff = parent->coords.x1 + pleft - area_tmp->x1
			- scroll_value->x;
	lv_coord_t right_diff = -(parent->coords.x2 - pright - area_tmp->x2
			- scroll_value->x);
