...
void lv_obj_redraw(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj) {
	...
	bool com_clip_res = _lv_area_intersect(&clip_coords_for_obj, clip_area_ori,
			&obj_coords_ext);
...
void _lv_disp_refr_timer(lv_timer_t *tmr) {
	...
	if (disp_refr->act_scr == NULL) {
		disp_refr->inv_p = 0;
		LV_LOG_WARN("there is no active screen"); REFR_TRACE("finished");
		return;
	}
...
static void refr_area(const lv_area_t *area_p) {
	...
	lv_coord_t y2 =
			area_p->y2 >= lv_disp_get_ver_res(disp_refr) ?
...
static lv_res_t layer_get_area(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj,
		lv_layer_type_t layer_type, lv_area_t *layer_area_out) {
	lv_coord_t ext_draw_size = _lv_obj_get_ext_draw_size(obj);
	lv_area_t obj_coords_ext;
	lv_obj_get_coords(obj, &obj_coords_ext);
	lv_area_increase(&obj_coords_ext, ext_draw_size, ext_draw_size);
	...
	if (layer_type == LV_LAYER_TYPE_TRANSFORM) {
		...
		lv_obj_get_transformed_area(obj, &inverse_clip_coords_for_obj, false,
				true);
...
static void layer_alpha_test(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx,
	...
	if (flags & LV_DRAW_LAYER_FLAG_HAS_ALPHA) {
		...
		if (has_alpha) {
			layer_ctx->area_act.y2 = layer_ctx->area_act.y1
					+ layer_ctx->max_row_with_alpha - 1;
		}
	} else {
		has_alpha = false;
	}
	...
	lv_draw_layer_adjust(draw_ctx, layer_ctx,
			has_alpha ? LV_DRAW_LAYER_FLAG_HAS_ALPHA : LV_DRAW_LAYER_FLAG_NONE);
}
...
void refr_obj(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj) {
	...
	if (layer_type == LV_LAYER_TYPE_NONE) {
		...
		lv_res_t res = layer_get_area(draw_ctx, obj, layer_type,
				&layer_area_full);
		if (res != LV_RES_OK)
			return;
		...
		lv_draw_layer_ctx_t *layer_ctx = lv_draw_layer_create(draw_ctx,
				&layer_area_full, flags);
		if (layer_ctx == NULL) {
...
static uint32_t get_max_row(lv_disp_t *disp, lv_coord_t area_w,
	...
	if (disp_refr->driver->rounder_cb) {
		...
		if (h_tmp <= 0) {
			LV_LOG_WARN(
					"Can't set draw_buf height using the round function. (Wrong round_cb or to " "small draw_buf)");
			return 0;
		} else {
			max_row = tmp.y2 + 1;
		}
...
static LV_ATTRIBUTE_FAST_MEM void draw_buf_rotate_90(bool invert_i,
		lv_coord_t area_w, lv_coord_t area_h, lv_color_t *orig_color_p,
		lv_color_t *rot_buf) {
	...
	if (drv->rotated == LV_DISP_ROT_180) {
		...
		while (row < area_h) {
			...
			if ((row == 0) && (area_h >= area_w)) {
				...
				draw_buf_rotate_90(drv->rotated == LV_DISP_ROT_270, area_w,
						height, color_p, rot_buf);
...
static void call_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area,
	...
	lv_area_t offset_area = { .x1 = area->x1 + drv->offset_x, .y1 = area->y1
			+ drv->offset_y, .x2 = area->x2 + drv->offset_x, .y2 = area->y2
			+ drv->offset_y };
