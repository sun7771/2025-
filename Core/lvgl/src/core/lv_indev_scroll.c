...
static lv_coord_t elastic_diff(lv_obj_t *scroll_obj, lv_coord_t diff,
		lv_coord_t scroll_start, lv_coord_t scroll_end, lv_dir_t dir);
...
void _lv_indev_scroll_handler(_lv_indev_proc_t *proc) {
	...
	if (proc->types.pointer.vect.x != 0 || proc->types.pointer.vect.y != 0) {
		...
		if (proc->types.pointer.scroll_dir == LV_DIR_HOR) {
			...
			diff_x = elastic_diff(scroll_obj, proc->types.pointer.vect.x, sl,
					sr, LV_DIR_HOR);
		} else {
			lv_coord_t st = lv_obj_get_scroll_top(scroll_obj);
			lv_coord_t sb = lv_obj_get_scroll_bottom(scroll_obj);
			diff_y = elastic_diff(scroll_obj, proc->types.pointer.vect.y, st,
					sb, LV_DIR_VER);
		}
...
void _lv_indev_scroll_throw_handler(_lv_indev_proc_t *proc) {
	...
	if (proc->types.pointer.scroll_dir == LV_DIR_VER) {
		...
		if (align_y == LV_SCROLL_SNAP_NONE) {
			...
			proc->types.pointer.scroll_throw_vect.y = elastic_diff(scroll_obj,
					proc->types.pointer.scroll_throw_vect.y, st, sb,
					LV_DIR_VER);

			lv_obj_scroll_by(scroll_obj, 0,
					proc->types.pointer.scroll_throw_vect.y, LV_ANIM_OFF);
		}
		/*With snapping find the nearest snap point and scroll there*/
			...
			lv_coord_t y = find_snap_point_y(scroll_obj, LV_COORD_MIN,
					LV_COORD_MAX, diff_y);
		...
		if (align_x == LV_SCROLL_SNAP_NONE) {
			...
			proc->types.pointer.scroll_throw_vect.x = elastic_diff(scroll_obj,
					proc->types.pointer.scroll_throw_vect.x, sl, sr,
					LV_DIR_HOR);

			lv_obj_scroll_by(scroll_obj,
					proc->types.pointer.scroll_throw_vect.x, 0, LV_ANIM_OFF);
		}
		/*With snapping find the nearest snap point and scroll there*/
		else {
			...
			lv_coord_t x = find_snap_point_x(scroll_obj, LV_COORD_MIN,
					LV_COORD_MAX, diff_x);
...
static lv_obj_t* find_scroll_obj(_lv_indev_proc_t *proc) {
	...
	bool ver_en = false;
	if (LV_ABS(
			proc->types.pointer.scroll_sum.x) > LV_ABS(proc->types.pointer.scroll_sum.y)) {
		hor_en = true;
	} else {
		ver_en = true;
	}
	...
	while (obj_act) {
		...
		if ((left_en && proc->types.pointer.scroll_sum.x >= scroll_limit)
				|| (right_en
						&& proc->types.pointer.scroll_sum.x <= -scroll_limit)
				|| (up_en && proc->types.pointer.scroll_sum.y >= scroll_limit)
...
static void init_scroll_limits(_lv_indev_proc_t *proc) {
	...
	if (lv_obj_has_flag(obj, LV_OBJ_FLAG_SCROLL_ONE) == false) {
		lv_area_set(&proc->types.pointer.scroll_area, LV_COORD_MIN,
				LV_COORD_MIN, LV_COORD_MAX, LV_COORD_MAX);
	}
	/*With STOP limit the scrolling to the perv and next snap point*/
	else {
		switch (lv_obj_get_scroll_snap_y(obj)) {
		case LV_SCROLL_SNAP_START:
			proc->types.pointer.scroll_area.y1 = find_snap_point_y(obj,
					obj->coords.y1 + 1, LV_COORD_MAX, 0);
			proc->types.pointer.scroll_area.y2 = find_snap_point_y(obj,
					LV_COORD_MIN, obj->coords.y1 - 1, 0);
			break;
		case LV_SCROLL_SNAP_END:
			proc->types.pointer.scroll_area.y1 = find_snap_point_y(obj,
					obj->coords.y2, LV_COORD_MAX, 0);
			proc->types.pointer.scroll_area.y2 = find_snap_point_y(obj,
					LV_COORD_MIN, obj->coords.y2, 0);
			break;
		case LV_SCROLL_SNAP_CENTER: {
			lv_coord_t y_mid = obj->coords.y1
					+ lv_area_get_height(&obj->coords) / 2;
			proc->types.pointer.scroll_area.y1 = find_snap_point_y(obj,
					y_mid + 1, LV_COORD_MAX, 0);
			proc->types.pointer.scroll_area.y2 = find_snap_point_y(obj,
					LV_COORD_MIN, y_mid - 1, 0);
			break;
		}
		default:
		...
		case LV_SCROLL_SNAP_START:
			proc->types.pointer.scroll_area.x1 = find_snap_point_x(obj,
					obj->coords.x1, LV_COORD_MAX, 0);
			proc->types.pointer.scroll_area.x2 = find_snap_point_x(obj,
					LV_COORD_MIN, obj->coords.x1, 0);
			break;
		case LV_SCROLL_SNAP_END:
			proc->types.pointer.scroll_area.x1 = find_snap_point_x(obj,
					obj->coords.x2, LV_COORD_MAX, 0);
			proc->types.pointer.scroll_area.x2 = find_snap_point_x(obj,
					LV_COORD_MIN, obj->coords.x2, 0);
			break;
		case LV_SCROLL_SNAP_CENTER: {
			lv_coord_t x_mid = obj->coords.x1
					+ lv_area_get_width(&obj->coords) / 2;
			proc->types.pointer.scroll_area.x1 = find_snap_point_x(obj,
					x_mid + 1, LV_COORD_MAX, 0);
			proc->types.pointer.scroll_area.x2 = find_snap_point_x(obj,
					LV_COORD_MIN, x_mid - 1, 0);
			break;
		}
		default:
...
static lv_coord_t find_snap_point_x(const lv_obj_t *obj, lv_coord_t min,
	...
	for (i = 0; i < child_cnt; i++) {
		...
		if (lv_obj_has_flag_any(child,
				LV_OBJ_FLAG_HIDDEN | LV_OBJ_FLAG_FLOATING))
			continue;
		if (lv_obj_has_flag(child, LV_OBJ_FLAG_SNAPPABLE)) {
			...
			case LV_SCROLL_SNAP_CENTER:
				...
				x_parent = obj->coords.x1 + pad_left
						+ (lv_area_get_width(&obj->coords) - pad_left
								- pad_right) / 2;
				break;
			default:
				continue;
...
static lv_coord_t find_snap_point_y(const lv_obj_t *obj, lv_coord_t min,
	...
	for (i = 0; i < child_cnt; i++) {
		...
		if (lv_obj_has_flag(child, LV_OBJ_FLAG_SNAPPABLE)) {
			...
			case LV_SCROLL_SNAP_CENTER:
				y_child = child->coords.y1
						+ lv_area_get_height(&child->coords) / 2;
				y_parent = obj->coords.y1 + pad_top
						+ (lv_area_get_height(&obj->coords) - pad_top
								- pad_bottom) / 2;
				break;
			default:
				continue;
...
static lv_coord_t elastic_diff(lv_obj_t *scroll_obj, lv_coord_t diff,
	...
	if (lv_obj_has_flag(scroll_obj, LV_OBJ_FLAG_SCROLL_ELASTIC)) {
		...
		if (dir == LV_DIR_HOR) {
			lv_coord_t pad_left = lv_obj_get_style_pad_left(scroll_obj,
					LV_PART_MAIN);
			lv_coord_t pad_right = lv_obj_get_style_pad_right(scroll_obj,
					LV_PART_MAIN);

			switch (snap) {
			case LV_SCROLL_SNAP_CENTER:
				snap_point = pad_left
						+ (lv_area_get_width(&scroll_obj->coords) - pad_left
								- pad_right) / 2 + scroll_obj->coords.x1;
				act_obj_point = lv_area_get_width(&act_obj->coords) / 2
						+ act_obj->coords.x1;
				break;
			case LV_SCROLL_SNAP_START:
			...
			lv_coord_t pad_top = lv_obj_get_style_pad_top(scroll_obj,
					LV_PART_MAIN);
			lv_coord_t pad_bottom = lv_obj_get_style_pad_bottom(scroll_obj,
					LV_PART_MAIN);

			switch (snap) {
			case LV_SCROLL_SNAP_CENTER:
				snap_point = pad_top
						+ (lv_area_get_height(&scroll_obj->coords) - pad_top
								- pad_bottom) / 2 + scroll_obj->coords.y1;
				act_obj_point = lv_area_get_height(&act_obj->coords) / 2
						+ act_obj->coords.y1;
				break;
			case LV_SCROLL_SNAP_START:
