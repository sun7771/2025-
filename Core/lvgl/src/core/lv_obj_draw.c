	...
	if (draw_dsc->bg_opa != LV_OPA_TRANSP) {
		...
		if (draw_dsc->bg_opa > LV_OPA_MIN) {
			...
			if (grad && grad->dir != LV_GRAD_DIR_NONE) {
				...
				draw_dsc->bg_grad.dither = lv_obj_get_style_bg_dither_mode(obj,
						part);
			}
		}
	}
	...
	if (draw_dsc->border_width) {
		if (draw_dsc->border_opa != LV_OPA_TRANSP) {
			...
			if (draw_dsc->border_opa > LV_OPA_MIN) {
				...
				draw_dsc->border_color = lv_obj_get_style_border_color_filtered(
						obj, part);
			}
		}
	}
		...
		if (draw_dsc->bg_img_src) {
			...
			if (draw_dsc->bg_img_opa > LV_OPA_MIN) {
				if (lv_img_src_get_type(draw_dsc->bg_img_src)
						== LV_IMG_SRC_SYMBOL) {
					draw_dsc->bg_img_symbol_font = lv_obj_get_style_text_font(
							obj, part);
					...
					draw_dsc->bg_img_tiled = lv_obj_get_style_bg_img_tiled(obj,
							part);
				}
			}
		}
	...
	if (draw_dsc->shadow_opa) {
		...
		if (draw_dsc->shadow_width) {
			if (draw_dsc->shadow_opa > LV_OPA_MIN) {
				...
				if (draw_dsc->shadow_opa > LV_OPA_MIN) {
					draw_dsc->shadow_ofs_x = lv_obj_get_style_shadow_ofs_x(obj,
							part);
					draw_dsc->shadow_ofs_y = lv_obj_get_style_shadow_ofs_y(obj,
							part);
