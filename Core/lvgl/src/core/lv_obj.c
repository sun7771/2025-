...
const lv_obj_class_t lv_obj_class = { .constructor_cb = lv_obj_constructor,
		.destructor_cb = lv_obj_destructor, .event_cb = lv_obj_event,
		.width_def = LV_DPI_DEF, .height_def = LV_DPI_DEF, .editable =
				LV_OBJ_CLASS_EDITABLE_FALSE, .group_def =
				LV_OBJ_CLASS_GROUP_DEF_FALSE, .instance_size =
				(sizeof(lv_obj_t)), .base_class = NULL, };
...
#if LV_DRAW_COMPLEX
		if (lv_obj_get_style_clip_corner(obj, LV_PART_MAIN)) {
			lv_draw_mask_radius_param_t *param = lv_draw_mask_remove_custom(
					obj + 8);
			if (param) {
...
static lv_res_t scrollbar_init_draw_dsc(lv_obj_t *obj, lv_draw_rect_dsc_t *dsc) {
	...
	if (dsc->border_opa > LV_OPA_MIN) {
		dsc->border_width = lv_obj_get_style_border_width(obj,
				LV_PART_SCROLLBAR);
		if (dsc->border_width > 0) {
			dsc->border_color = lv_obj_get_style_border_color(obj,
					LV_PART_SCROLLBAR);
		} else {
	...
	if (dsc->shadow_opa > LV_OPA_MIN) {
		dsc->shadow_width = lv_obj_get_style_shadow_width(obj,
				LV_PART_SCROLLBAR);
		if (dsc->shadow_width > 0) {
			dsc->shadow_spread = lv_obj_get_style_shadow_spread(obj,
					LV_PART_SCROLLBAR);
			dsc->shadow_color = lv_obj_get_style_shadow_color(obj,
					LV_PART_SCROLLBAR);
		} else {
	...
	if (code == LV_EVENT_PRESSED) {
		...
		if (lv_obj_has_flag(obj, LV_OBJ_FLAG_CHECKABLE)) {
			...
			if (c == LV_KEY_DOWN) {
				...
				lv_obj_scroll_to_y(obj,
						lv_obj_get_scroll_y(obj) + lv_obj_get_height(obj) / 4,
						anim_enable);
			} else if (c == LV_KEY_UP) {
				lv_obj_scroll_to_y(obj,
						lv_obj_get_scroll_y(obj) - lv_obj_get_height(obj) / 4,
				...
				if (!((lv_obj_get_scroll_dir(obj) & LV_DIR_HOR)
					...
					lv_obj_scroll_to_y(obj,
							lv_obj_get_scroll_y(obj)
									+ lv_obj_get_height(obj) / 4, anim_enable);
				else
					lv_obj_scroll_to_x(obj,
							lv_obj_get_scroll_x(obj)
									+ lv_obj_get_width(obj) / 4, anim_enable);
...
static void lv_obj_set_state(lv_obj_t *obj, lv_state_t new_state) {
	...
	for (i = 0; i < obj->style_cnt && tsi < STYLE_TRANSITION_MAX; i++) {
		...
		lv_part_t part_act = lv_obj_style_get_selector_part(
				obj->styles[i].selector);
