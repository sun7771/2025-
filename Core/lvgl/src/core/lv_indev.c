		...
		if (i->proc.long_pr_sent == 0
			...
			if (data->key == LV_KEY_ENTER) {
				...
				/*If not editable then just send a long press Call the ancestor's event handler*/
					...
					lv_event_send(indev_obj_act, LV_EVENT_LONG_PRESSED,
							indev_act);
					if (indev_reset_check(&i->proc))
						return;
				}
	...
	/*Release happened*/
		...
		if (data->key == LV_KEY_ENTER) {
			...
			if (editable_or_scrollable == false) {
				...
				if (i->proc.long_pr_sent == 0)
					lv_event_send(indev_obj_act, LV_EVENT_SHORT_CLICKED,
							indev_act);
				if (indev_reset_check(&i->proc))
					return;
...
static void indev_button_proc(lv_indev_t *i, lv_indev_data_t *data) {
	...
	if (prev_state != data->state) {
		if (data->state == LV_INDEV_STATE_PRESSED) {
			LV_LOG_INFO("button %" LV_PRIu32 " is pressed (x:%d y:%d)",
					data->btn_id, x, y);
		} else {
			LV_LOG_INFO("button %" LV_PRIu32 " is released (x:%d y:%d)",
					data->btn_id, x, y);
		}
	}
...
static void indev_proc_press(_lv_indev_proc_t *proc) {
	...
	proc->types.pointer.scroll_throw_vect.x =
			(proc->types.pointer.scroll_throw_vect.x
					+ proc->types.pointer.vect.x) / 2;
	proc->types.pointer.scroll_throw_vect.y =
			(proc->types.pointer.scroll_throw_vect.y
					+ proc->types.pointer.vect.y) / 2;
...
static void indev_proc_release(_lv_indev_proc_t *proc) {
	if (proc->wait_until_release != 0) {
		lv_event_send(proc->types.pointer.act_obj, LV_EVENT_PRESS_LOST,
				indev_act);
		if (indev_reset_check(proc))
			return;
...
static void indev_click_focus(_lv_indev_proc_t *proc) {
	...
	lv_group_t *g_prev =
			proc->types.pointer.last_pressed ?
					lv_obj_get_group(proc->types.pointer.last_pressed) : NULL;
...
void indev_gesture(_lv_indev_proc_t *proc) {
	...
	if ((LV_ABS(proc->types.pointer.gesture_sum.x)
		...
		if (LV_ABS(
				proc->types.pointer.gesture_sum.x) > LV_ABS(proc->types.pointer.gesture_sum.y)) {
