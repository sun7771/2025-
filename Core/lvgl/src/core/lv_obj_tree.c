...
void lv_obj_set_parent(lv_obj_t *obj, lv_obj_t *parent) {
	...
	for (i = lv_obj_get_index(obj);
		...
		old_parent->spec_attr->children[i] = old_parent->spec_attr->children[i
				+ 1];
	}
	old_parent->spec_attr->child_cnt--;
	if (old_parent->spec_attr->child_cnt) {
		old_parent->spec_attr->children = lv_mem_realloc(
...
static void obj_del_core(lv_obj_t *obj) {
	...
	if (obj->parent == NULL) {
		...
		disp->screen_cnt--;
		disp->screens = lv_mem_realloc(disp->screens,
				disp->screen_cnt * sizeof(lv_obj_t*));
	}
