...
bool lv_obj_refr_size(lv_obj_t *obj) {
	...
	if (obj->w_layout) {
		...
		if (w_is_content) {
			...
			if (parent->w_layout
					== 0&& lv_obj_get_style_width(parent, 0) == LV_SIZE_CONTENT) {
	...
	if (obj->h_layout) {
		...
		if (h_is_content) {
			 ...
			 *a circular dependency will occur. To solve it keep child size at zero */
			if (parent->h_layout
					== 0&& lv_obj_get_style_height(parent, 0) == LV_SIZE_CONTENT) {
				lv_coord_t border_w = lv_obj_get_style_border_width(obj, 0);
...
uint32_t lv_layout_register(lv_layout_update_cb_t cb, void *user_data) {
	layout_cnt++;
	LV_GC_ROOT(_lv_layout_list) = lv_mem_realloc(LV_GC_ROOT(_lv_layout_list),
...
void lv_obj_transform_point(const lv_obj_t *obj, lv_point_t *p, bool recursive,
	...
	if (obj) {
		...
		if (inv) {
			if (recursive)
				lv_obj_transform_point(lv_obj_get_parent(obj), p, recursive,
						inv);
			if (do_tranf)
...
void lv_obj_get_transformed_area(const lv_obj_t *obj, lv_area_t *area,
		bool recursive,
		bool inv) {
	lv_point_t p[4] = { { area->x1, area->y1 }, { area->x1, area->y2 }, {
			...
			if (!lv_obj_is_layout_positioned(child)) {
				...
				default:
					...
					if (lv_obj_get_style_y(child, 0) == 0) {
						child_res = LV_MAX(child_res,
								lv_area_get_width(&child->coords) + pad_left);
					}
				}
			} else {
				child_res = LV_MAX(child_res,
	...
	for (i = 0; i < child_cnt; i++) {
		...
		if (!lv_obj_is_layout_positioned(child)) {
			...
			default:
				...
				if (lv_obj_get_style_y(child, 0) == 0) {
					child_res = LV_MAX(child_res,
							lv_area_get_height(&child->coords) + pad_top);
				}
				break;
			}
		} else {
...
static void layout_update_core(lv_obj_t *obj) {
	...
	if (child_cnt > 0) {
		...
		if (layout_id > 0 && layout_id <= layout_cnt) {
			void *user_data =
					LV_GC_ROOT(_lv_layout_list)[layout_id - 1].user_data;
