...
lv_disp_t* lv_disp_drv_register(lv_disp_drv_t *driver) {
	...
	disp->refr_timer = lv_timer_create(_lv_disp_refr_timer,
			LV_DISP_DEF_REFR_PERIOD, disp);
	...
	if (lv_theme_default_is_inited() == false) {
		disp->theme = lv_theme_default_init(disp,
	...
	if (disp == NULL) {
		...
		case LV_DISP_ROT_270:
			return disp->driver->physical_ver_res > 0 ?
					disp->driver->physical_ver_res : disp->driver->ver_res;
		default:
			return disp->driver->physical_hor_res > 0 ?
					disp->driver->physical_hor_res : disp->driver->hor_res;
		}
	}
}
...
static void set_px_true_color_alpha(lv_disp_drv_t *disp_drv, uint8_t *buf,
	...
	lv_color_mix_with_alpha(bg_color, bg_opa, color, opa, &res_color,
			&buf_px[2]);
