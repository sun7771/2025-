...
const lv_obj_class_t lv_menu_class = { .constructor_cb = lv_menu_constructor,
		.destructor_cb = lv_menu_destructor, .base_class = &lv_obj_class,
		.width_def = (LV_DPI_DEF * 3) / 2, .height_def = LV_DPI_DEF * 2,
		.instance_size = sizeof(lv_menu_t) };
const lv_obj_class_t lv_menu_page_class = { .constructor_cb =
		lv_menu_page_constructor, .destructor_cb = lv_menu_page_destructor,
		.base_class = &lv_obj_class, .width_def = LV_PCT(100), .height_def =
				LV_SIZE_CONTENT, .instance_size = sizeof(lv_menu_page_t) };

const lv_obj_class_t lv_menu_cont_class = { .constructor_cb =
		lv_menu_cont_constructor, .base_class = &lv_obj_class, .width_def =
		LV_PCT(100), .height_def = LV_SIZE_CONTENT };

const lv_obj_class_t lv_menu_section_class = { .constructor_cb =
		lv_menu_section_constructor, .base_class = &lv_obj_class, .width_def =
		LV_PCT(100), .height_def = LV_SIZE_CONTENT };
...
void lv_menu_set_sidebar_page(lv_obj_t *obj, lv_obj_t *page) {
	...
	if (page != NULL) {
		...
		if (!menu->sidebar_generated) {
			...
			lv_obj_t *sidebar_header_back_btn = lv_btn_create(
					menu->sidebar_header);
			...
			lv_obj_t *sidebar_header_back_icon = lv_img_create(
					menu->sidebar_header_back_btn);
			lv_img_set_src(sidebar_header_back_icon, LV_SYMBOL_LEFT);

			lv_obj_t *sidebar_header_title = lv_label_create(
					menu->sidebar_header);
...
void lv_menu_set_mode_root_back_btn(lv_obj_t *obj,
...
void lv_menu_set_load_page_event(lv_obj_t *menu, lv_obj_t *obj, lv_obj_t *page) {
	...
	lv_menu_load_page_event_data_t *event_data = lv_mem_alloc(
			sizeof(lv_menu_load_page_event_data_t));
...
static void lv_menu_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
	...
	lv_obj_t *main_cont = lv_obj_class_create_obj(&lv_menu_main_cont_class,
			obj);
	...
	lv_obj_set_flex_align(main_header, LV_FLEX_ALIGN_START,
			LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
...
static void lv_menu_value_changed_event_cb(lv_event_t *e) {
	...
	lv_menu_page_t *main_page = (lv_menu_page_t*) lv_menu_get_cur_main_page(
			obj);
