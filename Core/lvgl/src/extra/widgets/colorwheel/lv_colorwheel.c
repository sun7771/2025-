 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_colorwheel_class = { .instance_size =
		sizeof(lv_colorwheel_t), .base_class = &lv_obj_class, .constructor_cb =
		lv_colorwheel_constructor, .event_cb = lv_colorwheel_event, .width_def =
		LV_DPI_DEF * 2, .height_def = LV_DPI_DEF * 2, .editable =
...
static void draw_disc_grad(lv_event_t *e) {
	...
	lv_draw_mask_radius_init(&mask_in_param, &mask_area, LV_RADIUS_CIRCLE,
			true);
	...
	if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
		...
		*s = LV_MAX(*s, knob_pad);
		...
		if (c == LV_KEY_RIGHT || c == LV_KEY_UP) {
			...
			case LV_COLORWHEEL_MODE_HUE:
				hsv_cur.h =
						colorwheel->hsv.h > 0 ? (colorwheel->hsv.h - 1) : 360;
				break;
			case LV_COLORWHEEL_MODE_SATURATION:
				hsv_cur.s =
						colorwheel->hsv.s > 0 ? (colorwheel->hsv.s - 1) : 100;
				break;
			case LV_COLORWHEEL_MODE_VALUE:
				hsv_cur.v =
						colorwheel->hsv.v > 0 ? (colorwheel->hsv.v - 1) : 100;
				break;
			}
		...
		info->res = _lv_area_is_point_on(&obj->coords, info->point,
				LV_RADIUS_CIRCLE);
...
static void refr_knob_pos(lv_obj_t *obj) {
	...
	colorwheel->knob.pos.x = (((int32_t) r * lv_trigo_sin(angle))
			>> LV_TRIGO_SHIFT);
	colorwheel->knob.pos.y = (((int32_t) r * lv_trigo_cos(angle))
			>> LV_TRIGO_SHIFT);
