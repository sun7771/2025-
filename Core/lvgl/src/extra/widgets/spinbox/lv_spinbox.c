...
const lv_obj_class_t lv_spinbox_class = { .constructor_cb =
		lv_spinbox_constructor, .event_cb = lv_spinbox_event, .width_def =
		LV_DPI_DEF, .instance_size = sizeof(lv_spinbox_t), .editable =
		LV_OBJ_CLASS_EDITABLE_TRUE, .base_class = &lv_textarea_class };
/**********************
 *      MACROS
 **********************/
...
void lv_spinbox_set_cursor_pos(lv_obj_t *obj, uint8_t pos) {
	...
	int32_t step_limit;
	step_limit =
			LV_MAX(spinbox->range_max,
					(spinbox->range_min < 0 ?
							(-spinbox->range_min) : spinbox->range_min));
				...
				if (spinbox->digit_count > 1) {
					if (spinbox->digit_step_dir == LV_DIR_RIGHT) {
						if (spinbox->step > 1) {
							...
							spinbox->step = lv_pow(10,
									spinbox->digit_count - 2);
							lv_spinbox_step_prev(obj);
						}
					} else {
						if (spinbox->step
								< lv_pow(10, spinbox->digit_count - 1)) {
							lv_spinbox_step_prev(obj);
						} else {
...
static void lv_spinbox_updatevalue(lv_obj_t *obj) {
	...
	intDigits =
			(spinbox->dec_point_pos == 0) ?
					spinbox->digit_count : spinbox->dec_point_pos;
