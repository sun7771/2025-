 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_calendar_class = { .constructor_cb =
		lv_calendar_constructor, .width_def = (LV_DPI_DEF * 3) / 2,
		.height_def = (LV_DPI_DEF * 3) / 2, .group_def =
				LV_OBJ_CLASS_GROUP_DEF_TRUE, .instance_size =
				sizeof(lv_calendar_t), .base_class = &lv_obj_class };
...
void lv_calendar_set_showed_date(lv_obj_t *obj, uint32_t year, uint32_t month) {
	...
	for (i = 0; i < 7; i++) {
		lv_btnmatrix_set_btn_ctrl(calendar->btnm, i,
...
lv_res_t lv_calendar_get_pressed_date(const lv_obj_t *obj,
	...
	const char *txt = lv_btnmatrix_get_btn_text(calendar->btnm,
			lv_btnmatrix_get_selected_btn(calendar->btnm));
...
static void draw_part_begin_event_cb(lv_event_t *e) {
	...
	if (dsc->part == LV_PART_ITEMS) {
		...
		if (lv_btnmatrix_has_btn_ctrl(obj, dsc->id,
...
static void highlight_update(lv_obj_t *obj) {
	...
	lv_btnmatrix_clear_btn_ctrl_all(calendar->btnm,
			LV_CALENDAR_CTRL_TODAY | LV_CALENDAR_CTRL_HIGHLIGHT);
		...
		for (i = 0; i < calendar->highlighted_dates_num; i++) {
			if (calendar->highlighted_dates[i].year
				...
				lv_btnmatrix_set_btn_ctrl(calendar->btnm,
						calendar->highlighted_dates[i].day - 1 + day_first + 7,
						LV_CALENDAR_CTRL_HIGHLIGHT);
			}
		}
	}
