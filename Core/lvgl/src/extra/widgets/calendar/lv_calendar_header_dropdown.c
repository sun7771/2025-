 ...
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t lv_calendar_header_dropdown_class = { .base_class =
		&lv_obj_class, .width_def = LV_PCT(100), .height_def = LV_SIZE_CONTENT,
...
lv_obj_t* lv_calendar_header_dropdown_create(lv_obj_t *parent) {
	lv_obj_t *obj = lv_obj_class_create_obj(&lv_calendar_header_dropdown_class,
			parent);
...
static void my_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
	...
	lv_obj_add_event_cb(year_dd, year_event_cb, LV_EVENT_VALUE_CHANGED,
			calendar);
