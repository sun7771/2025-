...
const lv_obj_class_t lv_meter_class = { .constructor_cb = lv_meter_constructor,
		.destructor_cb = lv_meter_destructor, .event_cb = lv_meter_event,
		.instance_size = sizeof(lv_meter_t), .base_class = &lv_obj_class };
...
void lv_meter_set_scale_major_ticks(lv_obj_t *obj, lv_meter_scale_t *scale,
		uint16_t nth, uint16_t width, uint16_t len, lv_color_t color,
		int16_t label_gap) {
...
lv_meter_indicator_t* lv_meter_add_needle_line(lv_obj_t *obj,
		lv_meter_scale_t *scale, uint16_t width, lv_color_t color,
		int16_t r_mod) {
	LV_ASSERT_OBJ(obj, MY_CLASS);
	lv_meter_t *meter = (lv_meter_t*) obj;
...
lv_meter_indicator_t* lv_meter_add_needle_img(lv_obj_t *obj,
		lv_meter_scale_t *scale, const void *src, lv_coord_t pivot_x,
		lv_coord_t pivot_y) {
	LV_ASSERT_OBJ(obj, MY_CLASS);
	lv_meter_t *meter = (lv_meter_t*) obj;
	lv_meter_indicator_t *indic = _lv_ll_ins_head(&meter->indicator_ll);
...
lv_meter_indicator_t* lv_meter_add_scale_lines(lv_obj_t *obj,
		lv_meter_scale_t *scale, lv_color_t color_start, lv_color_t color_end,
...
void lv_meter_set_indicator_start_value(lv_obj_t *obj,
		lv_meter_indicator_t *indic, int32_t value) {
...
static void draw_arcs(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx,
	...
	_LV_LL_READ_BACK(&meter->indicator_ll, indic)
	{
		...
		arc_dsc.opa =
				indic->opa > LV_OPA_MAX ?
						opa_main : (opa_main * indic->opa) >> 8;
...
static void draw_ticks_and_labels(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx,
	...
	_LV_LL_READ_BACK(&meter->scale_ll, scale)
	{
		...
		lv_draw_mask_radius_init(&outer_mask, &area_outer, LV_RADIUS_CIRCLE,
				false);
		...
		for (i = 0; i < scale->tick_cnt; i++) {
			...
			int32_t value_of_line = lv_map(i, 0, scale->tick_cnt - 1,
					scale->min, scale->max);
					...
					if (indic->type_data.scale_lines.color_start.full
						...
						if (indic->type_data.scale_lines.local_grad) {
							ratio = lv_map(value_of_line, indic->start_value,
									indic->end_value, LV_OPA_TRANSP,
									LV_OPA_COVER);
						} else {
							ratio = lv_map(value_of_line, scale->min,
									scale->max, LV_OPA_TRANSP, LV_OPA_COVER);
						}
						line_color = lv_color_mix(
								indic->type_data.scale_lines.color_end,
								indic->type_data.scale_lines.color_start,
								ratio);
					}
				}
			}
			...
			int32_t angle_upscale = ((i * scale->angle_range) * 10)
					/ (scale->tick_cnt - 1) + +scale->rotation * 10;
			...
			inner_act_mask_id = lv_draw_mask_add(
					major ? &inner_major_mask : &inner_minor_mask, NULL);
...
static void draw_needles(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx,
	...
	_LV_LL_READ_BACK(&meter->indicator_ll, indic)
	{
		...
		if (indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_LINE) {
			...
			lv_event_send(obj, LV_EVENT_DRAW_PART_BEGIN, &part_draw_dsc);
			lv_draw_img(draw_ctx, &img_dsc, &a,
...
static void inv_arc(lv_obj_t *obj, lv_meter_indicator_t *indic,
	...
	lv_draw_arc_get_area(scale_center.x, scale_center.y, r_out,
			LV_MIN(start_angle, end_angle), LV_MAX(start_angle, end_angle),
...
static void inv_line(lv_obj_t *obj, lv_meter_indicator_t *indic, int32_t value) {
	...
	if (indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_LINE) {
		...
		_lv_img_buf_get_transformed_area(&a, info.w, info.h, angle,
				LV_IMG_ZOOM_NONE, &indic->type_data.needle_img.pivot);
