...
const lv_obj_class_t lv_tileview_class = { .constructor_cb =
		lv_tileview_constructor, .base_class = &lv_obj_class, .instance_size =
		sizeof(lv_tileview_t) };

const lv_obj_class_t lv_tileview_tile_class = { .constructor_cb =
		lv_tileview_tile_constructor, .base_class = &lv_obj_class,
...
void lv_obj_set_tile_id(lv_obj_t *tv, uint32_t col_id, uint32_t row_id,
	...
	LV_LOG_WARN("No tile found with at (%d,%d) index", (int)col_id,
			(int)row_id);
}
