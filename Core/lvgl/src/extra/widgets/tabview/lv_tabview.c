...
const lv_obj_class_t lv_tabview_class = { .constructor_cb =
		lv_tabview_constructor, .destructor_cb = lv_tabview_destructor,
		.event_cb = lv_tabview_event, .width_def = LV_PCT(100), .height_def =
				LV_PCT(100), .base_class = &lv_obj_class, .instance_size =
				sizeof(lv_tabview_t) };
...
lv_obj_t* lv_tabview_add_tab(lv_obj_t *obj, const char *name) {
	...
	/*left or right dir*/
		...
		lv_memcpy_small(new_map, old_map,
	...
	lv_btnmatrix_set_btn_ctrl(btns, tabview->tab_cur,
...
static void lv_tabview_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
	...
	lv_obj_add_event_cb(btnm, btns_value_changed_event_cb,
