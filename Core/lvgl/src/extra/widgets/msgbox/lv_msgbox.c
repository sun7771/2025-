...
lv_obj_t* lv_msgbox_create(lv_obj_t *parent, const char *title, const char *txt,
	...
	if (add_close_btn) {
		...
		const lv_font_t *font = lv_obj_get_style_text_font(mbox->close_btn,
				LV_PART_MAIN);
	...
	if (btn_txts) {
		...
		const lv_font_t *font = lv_obj_get_style_text_font(mbox->btns,
				LV_PART_ITEMS);
