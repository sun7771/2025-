...
const lv_obj_class_t lv_spangroup_class = { .base_class = &lv_obj_class,
		.constructor_cb = lv_spangroup_constructor, .destructor_cb =
				lv_spangroup_destructor, .event_cb = lv_spangroup_event,
		.instance_size = sizeof(lv_spangroup_t), .width_def = LV_SIZE_CONTENT,
...
uint32_t lv_spangroup_get_expand_width(lv_obj_t *obj, uint32_t max_width) {
	...
	_LV_LL_READ(&spans->child_ll, cur_span)
	{
		...
		while (cur_txt[j] != '\0') {
			...
			uint16_t letter_w = lv_font_get_glyph_width(font, letter,
					letter_next);
	...
	while (cur_span) {
		...
		while (1) {
			...
			if (cur_txt_ofs == 0) {
				...
				snippet.letter_space = lv_span_get_style_text_letter_space(obj,
						cur_span);
				snippet.line_h = lv_font_get_line_height(snippet.font)
						+ line_space;
			}
			...
			bool isfill = lv_txt_get_snippet(&cur_txt[cur_txt_ofs],
					snippet.font, snippet.letter_space, max_w, txt_flag,
					&use_width, &next_ofs);
			...
			if (isfill && next_ofs > 0 && snippet_cnt > 0) {
				...
				uint32_t letter = _lv_txt_encoded_prev(&cur_txt[cur_txt_ofs],
						&tmp_ofs);
				if (!(letter == '\0' || letter == '\n' || letter == '\r'
						|| _lv_txt_is_break_char(letter))) {
					tmp_ofs = 0;
					letter = _lv_txt_encoded_next(
							&cur_txt[cur_txt_ofs + next_ofs], &tmp_ofs);
					if (!(letter == '\0' || letter == '\n' || letter == '\r'
							|| _lv_txt_is_break_char(letter))) {
						break;
					}
...
static void lv_spangroup_event(const lv_obj_class_t *class_p, lv_event_t *e) {
	...
	if (code == LV_EVENT_DRAW_MAIN) {
		...
		if (spans->mode == LV_SPAN_MODE_EXPAND) {
			if (spans->refresh) {
				spans->cache_w = (lv_coord_t) lv_spangroup_get_expand_width(obj,
						0);
...
static int32_t lv_span_get_style_text_decor(lv_obj_t *par, lv_span_t *span) {
	...
	if (res != LV_RES_OK) {
		decor = (lv_text_decor_t) lv_obj_get_style_text_decor(par,
				LV_PART_MAIN);
		;
	} else {
	...
	while (cur_span) {
		...
		/* Whether the current line is the end line and does overflow processing */
		{
			...
			if (last_snippet->txt[last_snippet->bytes] == '\0') {
				next_line_h = 0;
				lv_span_t *next_span = _lv_ll_get_next(&spans->child_ll,
						last_snippet->span);
			...
			if (txt_pos.y + max_line_h + next_line_h - line_space
				...
				if (last_snippet->txt[last_snippet->bytes] != '\0') {
					...
					last_snippet->txt_w = lv_txt_get_width(last_snippet->txt,
							last_snippet->bytes, last_snippet->font,
		...
		for (i = 0; i < item_cnt; i++) {
			...
			label_draw_dsc.color = lv_span_get_style_text_color(obj,
					pinfo->span);
			...
			label_draw_dsc.blend_mode = lv_span_get_style_text_blend_mode(obj,
					pinfo->span);
			if (obj_opa < LV_OPA_MAX) {
				label_draw_dsc.opa = (uint16_t) ((uint16_t) label_draw_dsc.opa
						* obj_opa) >> 8;
			}
			...
			while (j < txt_bytes) {
				...
				int32_t letter_w = lv_font_get_glyph_width(pinfo->font, letter,
						letter_next);
			...
			/* draw decor */
			lv_text_decor_t decor = lv_span_get_style_text_decor(obj,
					pinfo->span);
			if (decor != LV_TEXT_DECOR_NONE) {
				...
				line_dsc.width =
						label_draw_dsc.font->underline_thickness ?
				...
				if (decor & LV_TEXT_DECOR_UNDERLINE) {
					...
					lv_point_t p2;
					p1.x = txt_pos.x;
					p1.y = pos.y + pinfo->line_h - line_space
