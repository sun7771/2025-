...
const lv_obj_class_t lv_chart_class = { .constructor_cb = lv_chart_constructor,
		.destructor_cb = lv_chart_destructor, .event_cb = lv_chart_event,
		.width_def = LV_PCT(100), .height_def = LV_DPI_DEF * 2, .instance_size =
				sizeof(lv_chart_t), .base_class = &lv_obj_class };
...
void lv_chart_set_axis_tick(lv_obj_t *obj, lv_chart_axis_t axis,
		lv_coord_t major_len, lv_coord_t minor_len, lv_coord_t major_cnt,
...
static void draw_div_lines(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
	...
	lv_border_side_t border_side = lv_obj_get_style_border_side(obj,
			LV_PART_MAIN);
...
static void draw_series_line(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
	lv_area_t clip_area;
	if (_lv_area_intersect(&clip_area, &obj->coords,
			draw_ctx->clip_area) == false)
		return;
...
static void draw_series_bar(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
	...
	int32_t ser_gap = ((int32_t) lv_obj_get_style_pad_column(obj, LV_PART_ITEMS)
			* chart->zoom_x) >> 8; /*Gap between the columns on the ~same X*/
	...
	for (i = 0; i < chart->point_cnt; i++) {
		lv_coord_t x_act = (int32_t) ((int32_t) (w - block_w) * i)
				/ (chart->point_cnt - 1) + obj->coords.x1 + x_ofs;
	...
	_LV_LL_READ_BACK(&chart->cursor_ll, cursor)
	{
		...
		if (cursor->pos_set) {
			...
			if (cursor->point_id == LV_CHART_POINT_NONE)
				continue;
			lv_point_t p;
			lv_chart_get_point_pos_by_id(obj, cursor->ser, cursor->point_id,
					&p);
			cx = p.x;
			cy = p.y;
		}
...
static void draw_x_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx,
	...
	if (axis == LV_CHART_AXIS_PRIMARY_X) {
		label_gap =
				t->label_en ?
						lv_obj_get_style_pad_bottom(obj, LV_PART_TICKS) : 0;
		y_ofs = obj->coords.y2;
	} else {
		label_gap =
