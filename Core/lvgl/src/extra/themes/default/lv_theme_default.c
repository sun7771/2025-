...
typedef enum {
	DISP_SMALL = 3, DISP_MEDIUM = 2, DISP_LARGE = 1,
} disp_size_t;
...
static void style_init(void) {
	static const lv_style_prop_t trans_props[] = { LV_STYLE_BG_OPA,
			LV_STYLE_BG_COLOR, LV_STYLE_TRANSFORM_WIDTH,
			LV_STYLE_TRANSFORM_HEIGHT, LV_STYLE_TRANSLATE_Y,
			LV_STYLE_TRANSLATE_X, LV_STYLE_TRANSFORM_ZOOM,
			LV_STYLE_TRANSFORM_ANGLE, LV_STYLE_COLOR_FILTER_OPA,
	...
	lv_style_transition_dsc_init(&trans_delayed, trans_props,
			lv_anim_path_linear, TRANSITION_TIME, 70, NULL);

	static lv_style_transition_dsc_t trans_normal;
	lv_style_transition_dsc_init(&trans_normal, trans_props,
			lv_anim_path_linear, TRANSITION_TIME, 0, NULL);
	...
	lv_style_set_radius(&styles->btn,
			(disp_size == DISP_LARGE ? lv_disp_dpx(theme.disp, 16) :
				disp_size == DISP_MEDIUM ?
						lv_disp_dpx(theme.disp, 12) : lv_disp_dpx(theme.disp, 8)));
	...
	lv_style_set_radius(&styles->keyboard_btn_bg,
			disp_size == DISP_SMALL ? RADIUS_DEFAULT / 2 : RADIUS_DEFAULT);
#endif
...
lv_theme_t* lv_theme_default_init(lv_disp_t *disp, lv_color_t color_primary,
	...
	if (!lv_theme_default_is_inited()) {
		inited = false;
		LV_GC_ROOT(_lv_theme_default_styles) = lv_mem_alloc(
				sizeof(my_theme_styles_t));
...
static void theme_apply(lv_theme_t *th, lv_obj_t *obj) {
	...
	if (lv_obj_check_type(obj, &lv_obj_class)) {
		...
		if (lv_obj_check_type(lv_obj_get_parent(obj), &lv_tabview_class)) {
			...
			lv_obj_add_style(obj, &styles->bg_color_primary_muted,
					LV_PART_ITEMS | LV_STATE_CHECKED);
			lv_obj_add_style(obj, &styles->tab_btn,
					LV_PART_ITEMS | LV_STATE_CHECKED);
...
#if LV_THEME_DEFAULT_GROW
		lv_obj_add_style(obj, &styles->grow,
				LV_PART_INDICATOR | LV_STATE_PRESSED);
#endif
		lv_obj_add_style(obj, &styles->transition_normal,
				LV_PART_INDICATOR | LV_STATE_PRESSED);
		...
		lv_obj_add_style(obj,
				disp_size == DISP_LARGE ?
						&styles->pad_small : &styles->pad_tiny, 0);
		lv_obj_add_style(obj, &styles->outline_primary, LV_STATE_FOCUS_KEY);
		lv_obj_add_style(obj, &styles->outline_secondary, LV_STATE_EDITED);
		lv_obj_add_style(obj, &styles->btn, LV_PART_ITEMS);
		lv_obj_add_style(obj, &styles->disabled,
