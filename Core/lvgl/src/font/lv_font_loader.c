...
static bool load_cmaps_tables(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
	...
	if (lv_fs_read(fp, cmap_table,
			font_dsc->cmap_num * sizeof(cmap_table_bin_t), NULL)
			!= LV_FS_RES_OK) {
		return false;
	}

	for (unsigned int i = 0; i < font_dsc->cmap_num; ++i) {
		lv_fs_res_t res = lv_fs_seek(fp,
				cmaps_start + cmap_table[i].data_offset, LV_FS_SEEK_SET);
		if (res != LV_FS_RES_OK) {
			return false;
		}
		...
		case LV_FONT_FMT_TXT_CMAP_SPARSE_TINY: {
			...
			if (cmap_table[i].format_type == LV_FONT_FMT_TXT_CMAP_SPARSE_FULL) {
				...
				if (lv_fs_read(fp, buf, sizeof(uint16_t) * cmap->list_length,
						NULL) != LV_FS_RES_OK) {
					return false;
				}
			}
...
static int32_t load_cmaps(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
	...
	cmap_table_bin_t *cmaps_tables = lv_mem_alloc(
			sizeof(cmap_table_bin_t) * font_dsc->cmap_num);
...
static int32_t load_glyph(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
		uint32_t start, uint32_t *glyph_offset, uint32_t loca_count,
	...
	for (unsigned int i = 0; i < loca_count; ++i) {
		...
		int next_offset =
				(i < loca_count - 1) ?
						glyph_offset[i + 1] : (uint32_t) glyph_length;
	...
	uint8_t *glyph_bmp = (uint8_t*) lv_mem_alloc(
			sizeof(uint8_t) * cur_bmp_size);
	...
	for (unsigned int i = 1; i < loca_count; ++i) {
		...
		if (nbits % 8 == 0) { /*Fast path*/
			...
			glyph_bmp[cur_bmp_size + bmp_size - 1] = read_bits(&bit_it,
					8 - nbits % 8, &res);
			if (res != LV_FS_RES_OK) {
				return -1;
			}
			...
			glyph_bmp[cur_bmp_size + bmp_size - 1] = glyph_bmp[cur_bmp_size
					+ bmp_size - 1] << (nbits % 8);
...
static bool lvgl_load_font(lv_fs_file_t *fp, lv_font_t *font) {
	...
	int32_t kern_length = load_kern(fp, font_dsc, font_header.glyph_id_format,
			kern_start);
...
int32_t load_kern(lv_fs_file_t *fp, lv_font_fmt_txt_dsc_t *font_dsc,
	...
	if (0 == kern_format_type) { /*sorted pairs*/
		lv_font_fmt_txt_kern_pair_t *kern_pair = lv_mem_alloc(
		...
		lv_font_fmt_txt_kern_classes_t *kern_classes = lv_mem_alloc(
		...
		int kern_values_length = sizeof(int8_t) * kern_table_rows
				* kern_table_cols;
