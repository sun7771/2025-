 ...
 **********************/
typedef enum {
	RLE_STATE_SINGLE = 0, RLE_STATE_REPEATE, RLE_STATE_COUNTER,
} rle_state_t;
...
static uint32_t get_glyph_dsc_id(const lv_font_t *font, uint32_t letter) {
	...
	for (i = 0; i < fdsc->cmap_num; i++) {
		...
		if (fdsc->cmaps[i].type == LV_FONT_FMT_TXT_CMAP_FORMAT0_TINY) {
			...
			uint16_t key = rcp;
			uint16_t *p = _lv_utils_bsearch(&key, fdsc->cmaps[i].unicode_list,
...
static int8_t get_kern_value(const lv_font_t *font, uint32_t gid_left,
	...
	if (fdsc->kern_classes == 0) {
		...
		if (kdsc->glyph_ids_size == 0) {
			...
			uint16_t *kid_p = _lv_utils_bsearch(&g_id_both, g_ids,
			...
			uint32_t *kid_p = _lv_utils_bsearch(&g_id_both, g_ids,
