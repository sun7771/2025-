...
LV_ATTRIBUTE_FAST_MEM void lv_draw_label(lv_draw_ctx_t *draw_ctx,
	...
	bool clip_ok = _lv_area_intersect(&clipped_area, coords,
			draw_ctx->clip_area);
	if (!clip_ok)
		return;
	...
	uint32_t line_end = line_start
			+ _lv_txt_get_next_line(&txt[line_start], font, dsc->letter_space,
					w, NULL, dsc->flag);
	...
	while (pos.y + line_height_font < draw_ctx->clip_area->y1) {
		/*Go to next line*/
		line_start = line_end;
		line_end += _lv_txt_get_next_line(&txt[line_start], font,
	...
	if ((dsc->decor & LV_TEXT_DECOR_UNDERLINE)
		...
		line_dsc.width =
				font->underline_thickness ? font->underline_thickness : 1;
	...
	while (txt[line_start] != '\0') {
...
#else
				logical_char_pos = _lv_txt_encoded_get_char_id(txt,
						line_start + i);
#endif
			}
		...
		if (align == LV_TEXT_ALIGN_CENTER) {
			line_width = lv_txt_get_width(&txt[line_start],
					line_end - line_start, font, dsc->letter_space, dsc->flag);
