 ...
 *  STATIC PROTOTYPES
 **********************/
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t lv_draw_mask_line(
		lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len,
		lv_draw_mask_line_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t lv_draw_mask_radius(
		lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len,
		lv_draw_mask_radius_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t lv_draw_mask_angle(
		lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len,
		lv_draw_mask_angle_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t lv_draw_mask_fade(
		lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len,
		lv_draw_mask_fade_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t lv_draw_mask_map(
		lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len,
		lv_draw_mask_map_param_t *param);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t lv_draw_mask_polygon(
		lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len,
		lv_draw_mask_polygon_param_t *param);

LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t line_mask_flat(
		lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len,
		lv_draw_mask_line_param_t *p);
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t line_mask_steep(
		lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len,
		lv_draw_mask_line_param_t *p);
...
static lv_opa_t* get_next_line(_lv_draw_mask_radius_circle_dsc_t *c,
		lv_coord_t y, lv_coord_t *len, lv_coord_t *x_start);
...
LV_ATTRIBUTE_FAST_MEM lv_draw_mask_res_t lv_draw_mask_apply(lv_opa_t *mask_buf,
		lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len) {
	bool changed = false;
 ...
 * - `LV_DRAW_MASK_RES_CHANGED`: `mask_buf` has changed, it shows the desired opacity of each pixel in the given line
 */
LV_ATTRIBUTE_FAST_MEM lv_draw_mask_res_t lv_draw_mask_apply_ids(
		lv_opa_t *mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len,
...
void lv_draw_mask_angle_init(lv_draw_mask_angle_param_t *param,
		lv_coord_t vertex_x, lv_coord_t vertex_y, lv_coord_t start_angle,
	...
	lv_draw_mask_line_angle_init(&param->end_line, vertex_x, vertex_y,
			end_angle, end_side);
}
...
void lv_draw_mask_radius_init(lv_draw_mask_radius_param_t *param,
		const lv_area_t *rect, lv_coord_t radius, bool inv) {
...
void lv_draw_mask_fade_init(lv_draw_mask_fade_param_t *param,
		const lv_area_t *coords, lv_opa_t opa_top, lv_coord_t y_top,
...
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t lv_draw_mask_angle(
	...
	if (p->cfg.start_angle < 180 && p->cfg.end_angle < 180
		...
		if (p->cfg.start_angle == 180) {
			...
			res1 = lv_draw_mask_line(mask_buf, abs_x, abs_y, len,
					&p->start_line);
		}
	...
	if (outer == false) {
		for (i = 0; i < aa_len; i++) {
			...
			if (cir_x_right + i >= 0 && cir_x_right + i < len) {
				mask_buf[cir_x_right + i] = mask_mix(opa,
						mask_buf[cir_x_right + i]);
			}
			if (cir_x_left - i >= 0 && cir_x_left - i < len) {
				mask_buf[cir_x_left - i] = mask_mix(opa,
		...
		lv_coord_t clr_len = LV_CLAMP(0, cir_x_right - clr_start,
				len - clr_start);
...
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t lv_draw_mask_fade(
	...
	if (abs_y <= p->cfg.y_top) {
		...
		lv_opa_t opa_act = (int32_t) ((int32_t) (abs_y - p->cfg.y_top)
				* opa_diff) / y_diff;
...
LV_ATTRIBUTE_FAST_MEM static lv_draw_mask_res_t lv_draw_mask_polygon(
	...
	for (i = 0; i < param->cfg.point_cnt; i++) {
		...
		lv_point_t p2 = param->cfg.points[
				i + 1 < param->cfg.point_cnt ? i + 1 : 0];
	...
	lv_draw_mask_line_points_init(&line_p, lines[0].p1.x, lines[0].p1.y,
			lines[0].p2.x, lines[0].p2.y, LV_DRAW_MASK_LINE_SIDE_RIGHT);
	if (line_p.steep == 0 && line_p.flat) {
		...
		for (i = 0; i < len; i++) {
			mask_buf[i] = mask_mix(mask_buf[i],
					(abs_x + i >= x1 && abs_x + i <= x2) * 0xFF);
		}
		lv_draw_mask_free_param(&line_p);
		return LV_DRAW_MASK_RES_CHANGED;
	}
	lv_draw_mask_res_t res1 = lv_draw_mask_line(mask_buf, abs_x, abs_y, len,
			&line_p);
	lv_draw_mask_free_param(&line_p);
	if (res1 == LV_DRAW_MASK_RES_TRANSP) {
		return LV_DRAW_MASK_RES_TRANSP;
	...
	lv_draw_mask_line_points_init(&line_p, lines[1].p1.x, lines[1].p1.y,
			lines[1].p2.x, lines[1].p2.y, LV_DRAW_MASK_LINE_SIDE_LEFT);
	if (line_p.steep == 0 && line_p.flat) {
	...
	lv_draw_mask_res_t res2 = lv_draw_mask_line(mask_buf, abs_x, abs_y, len,
			&line_p);
	lv_draw_mask_free_param(&line_p);
	if (res2 == LV_DRAW_MASK_RES_TRANSP) {
		return LV_DRAW_MASK_RES_TRANSP;
...
static void circ_calc_aa4(_lv_draw_mask_radius_circle_dsc_t *c,
	...
	lv_coord_t *cir_x = lv_mem_buf_get(
			(radius + 1) * 2 * 2 * sizeof(lv_coord_t));
