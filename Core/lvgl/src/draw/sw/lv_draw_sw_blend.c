...
LV_ATTRIBUTE_FAST_MEM static void fill_normal(lv_color_t *dest_buf,
		const lv_area_t *dest_area, lv_coord_t dest_stride, lv_color_t color,
...
LV_ATTRIBUTE_FAST_MEM static void map_normal(lv_color_t *dest_buf,
		const lv_area_t *dest_area, lv_coord_t dest_stride,
...
static inline lv_color_t color_blend_true_color_additive(lv_color_t fg,
		lv_color_t bg, lv_opa_t opa);
static inline lv_color_t color_blend_true_color_subtractive(lv_color_t fg,
		lv_color_t bg, lv_opa_t opa);
static inline lv_color_t color_blend_true_color_multiply(lv_color_t fg,
		lv_color_t bg, lv_opa_t opa);
#endif /*LV_DRAW_COMPLEX*/
...
LV_ATTRIBUTE_FAST_MEM void lv_draw_sw_blend_basic(lv_draw_ctx_t *draw_ctx,
		const lv_draw_sw_blend_dsc_t *dsc) {
	const lv_opa_t *mask;
	...
	if (disp->driver->set_px_cb) {
		if (dsc->src_buf == NULL) {
			fill_set_px(dest_buf, &blend_area, dest_stride, dsc->color,
		...
		if (dsc->src_buf == NULL) {
			fill_normal(dest_buf, &blend_area, dest_stride, dsc->color,
					dsc->opa, mask, mask_stride);
		} else {
...
#if LV_DRAW_COMPLEX
		if (dsc->src_buf == NULL) {
			fill_blended(dest_buf, &blend_area, dest_stride, dsc->color,
	...
	if (mask == NULL) {
		for (y = blend_area->y1; y <= blend_area->y2; y++) {
			for (x = blend_area->x1; x <= blend_area->x2; x++) {
				disp->driver->set_px_cb(disp->driver, (void*) dest_buf,
						dest_stride, x, y, color, opa);
			}
		}
	} else {
		...
		for (y = 0; y < h; y++) {
			for (x = 0; x < w; x++) {
				if (mask[x]) {
					...
					disp->driver->set_px_cb(disp->driver, (void*) dest_buf,
							dest_stride, blend_area->x1 + x, blend_area->y1 + y,
							color, (uint32_t) ((uint32_t) opa * mask[x]) >> 8);
				}
			}
			mask += mask_stide;
		}
...
LV_ATTRIBUTE_FAST_MEM static void fill_normal(lv_color_t *dest_buf,
	...
	if (mask == NULL) {
		...
		/*Has opacity*/
			...
			lv_color_t last_res_color = lv_color_mix(color, last_dest_color,
					opa);
			...
			for (y = 0; y < h; y++) {
				for (x = 0; x < w; x++) {
					if (last_dest_color.full != dest_buf[x].full) {
						...
						last_res_color = lv_color_mix_premult(color_premult,
								dest_buf[x], opa_inv);
					}
					...
					if (*mask) {
						...
						if (*mask != last_mask
							...
							if (opa_tmp == LV_OPA_COVER)
								...
								last_res_color = lv_color_mix(color,
										dest_buf[x], opa_tmp);
							last_mask = *mask;
							last_dest_color.full = dest_buf[x].full;
						}
...
static void fill_blended(lv_color_t *dest_buf, const lv_area_t *dest_area,
	...
	/*Masked*/
		...
		lv_opa_t opa_tmp =
				mask[0] >= LV_OPA_MAX ?
						opa : (uint32_t) ((uint32_t) mask[0] * opa) >> 8;
...

{
	...
	if (mask == NULL) {
		for (y = 0; y < h; y++) {
			for (x = 0; x < w; x++) {
				disp->driver->set_px_cb(disp->driver, (void*) dest_buf,
						dest_stride, dest_area->x1 + x, dest_area->y1 + y,
						src_buf[x], opa);
			}
			src_buf += src_stride;
		}
				...
				for (x = 0; x < w; x++) {
					if (mask[x]) {
						lv_opa_t opa_tmp =
								mask[x] >= LV_OPA_MAX ?
										opa : ((opa * mask[x]) >> 8);
						dest_buf[x] = lv_color_mix(src_buf[x], dest_buf[x],
								opa_tmp);
					}
				}
				dest_buf += dest_stride;
...
static void map_blended(lv_color_t *dest_buf, const lv_area_t *dest_area,
	...
	if (mask == NULL) {
		...
		lv_color_t last_res_color = blend_fp(last_src_color, last_dest_color,
				opa);
		for (y = 0; y < h; y++) {
			for (x = 0; x < w; x++) {
				if (last_src_color.full != src_buf[x].full
					...
					last_res_color = blend_fp(last_src_color, last_dest_color,
							opa);
				}
				dest_buf[x] = last_res_color;
			}
