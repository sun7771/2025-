...
LV_ATTRIBUTE_FAST_MEM static void draw_line_skew(
		struct _lv_draw_ctx_t *draw_ctx, const lv_draw_line_dsc_t *dsc,
...
LV_ATTRIBUTE_FAST_MEM static void draw_line_hor(struct _lv_draw_ctx_t *draw_ctx,
	...
	bool is_common;
	is_common = _lv_area_intersect(&blend_area, &blend_area,
			draw_ctx->clip_area);
	if (!is_common)
		return;
	...
	/*If there other mask apply it*/
		...
		for (h = blend_area.y1; h <= y2; h++) {
			...
			blend_dsc.mask_res = lv_draw_mask_apply(mask_buf, blend_area.x1, h,
					blend_area_w);
...
LV_ATTRIBUTE_FAST_MEM static void draw_line_skew(
	...
	static const uint8_t wcorr[] = { 128, 128, 128, 129, 129, 130, 130, 131,
			132, 133, 134, 135, 137, 138, 140, 141, 143, 145, 147, 149, 151,
			153, 155, 158, 160, 162, 165, 167, 170, 173, 175, 178, 181, };
	...
	bool is_common = _lv_area_intersect(&blend_area, &blend_area,
			draw_ctx->clip_area);
	if (is_common == false)
		return;
	...
	if (flat) {
		if (xdiff > 0) {
			lv_draw_mask_line_points_init(&mask_left_param, p1.x,
					p1.y - w_half0, p2.x, p2.y - w_half0,
					LV_DRAW_MASK_LINE_SIDE_LEFT);
			lv_draw_mask_line_points_init(&mask_right_param, p1.x,
					p1.y + w_half1, p2.x, p2.y + w_half1,
		...
		lv_draw_mask_line_points_init(&mask_left_param, p1.x + w_half1, p1.y,
				p2.x + w_half1, p2.y, LV_DRAW_MASK_LINE_SIDE_LEFT);
		lv_draw_mask_line_points_init(&mask_right_param, p1.x - w_half0, p1.y,
				p2.x - w_half0, p2.y, LV_DRAW_MASK_LINE_SIDE_RIGHT);
	}
	...
	if (!dsc->raw_end) {
		...
		lv_draw_mask_line_points_init(&mask_bottom_param, p2.x, p2.y,
				p2.x - ydiff, p2.y + xdiff, LV_DRAW_MASK_LINE_SIDE_TOP);
	...
	int32_t h;
	uint32_t hor_res = (uint32_t) lv_disp_get_hor_res(
	...
	for (h = blend_area.y1; h <= y2; h++) {
		blend_dsc.mask_res = lv_draw_mask_apply(&mask_buf[mask_p],
				blend_area.x1, h, draw_area_w);
		if (blend_dsc.mask_res == LV_DRAW_MASK_RES_TRANSP) {
