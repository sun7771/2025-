...
LV_ATTRIBUTE_FAST_MEM void lv_draw_sw_img_decoded(
	...
	/*In the other cases every pixel need to be checked one-by-one*/
		...
		while (blend_area.y1 <= y_last) {
			...
			if (transform) {
				lv_draw_transform(draw_ctx, &transform_area, src_buf, src_w,
						src_h, src_w, draw_dsc, cf, rgb_buf, mask_buf);
			} else {
				convert_cb(&transform_area, src_buf, src_w, src_h, src_w,
			...
			if (draw_dsc->recolor_opa > LV_OPA_MIN) {
				...
				for (i = 0; i < buf_size; i++) {
					rgb_buf[i] = lv_color_mix_premult(premult_v, rgb_buf[i],
							recolor_opa);
				}
			}
#if LV_DRAW_COMPLEX
			...
			if (mask_any) {
				...
				for (y = blend_area.y1; y <= blend_area.y2; y++) {
					...
					mask_res_line = lv_draw_mask_apply(mask_buf_tmp,
							blend_area.x1, y, blend_w);
...
static void convert_cb(const lv_area_t *dest_area, const void *src_buf,
	...
	if (cf == LV_IMG_CF_TRUE_COLOR || cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) {
		...
		lv_coord_t src_new_line_step_px = (src_stride
				- lv_area_get_width(dest_area));
		lv_coord_t src_new_line_step_byte = src_new_line_step_px
