...
void lv_draw_sw_polygon(lv_draw_ctx_t *draw_ctx,
		const lv_draw_rect_dsc_t *draw_dsc, const lv_point_t *points,
		uint16_t point_cnt) {
#if LV_DRAW_COMPLEX
	if (point_cnt < 3)
		return;
	...
	lv_area_t poly_coords = { .x1 = LV_COORD_MAX, .y1 = LV_COORD_MAX, .x2 =
			LV_COORD_MIN, .y2 = LV_COORD_MIN };
	...
	bool is_common;
	lv_area_t clip_area;
	is_common = _lv_area_intersect(&clip_area, &poly_coords,
			draw_ctx->clip_area);
	if (!is_common) {
		lv_mem_buf_release(p);
		return;
	...
	lv_draw_mask_line_param_t *mp = lv_mem_buf_get(
			sizeof(lv_draw_mask_line_param_t) * point_cnt);
	...
	do {
		...
		if (p[i_next_left].y >= p[i_prev_left].y) {
			if (p[i_next_left].y != p[i_prev_left].y
				...
				lv_draw_mask_line_points_init(mp_next, p[i_prev_left].x,
						p[i_prev_left].y, p[i_next_left].x, p[i_next_left].y,
		...
		if (p[i_next_right].y >= p[i_prev_right].y) {
			if (p[i_next_right].y != p[i_prev_right].y
				...
				lv_draw_mask_line_points_init(mp_next, p[i_prev_right].x,
						p[i_prev_right].y, p[i_next_right].x, p[i_next_right].y,
