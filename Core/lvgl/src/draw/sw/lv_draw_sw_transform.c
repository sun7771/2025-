...
static void argb_no_aa(const uint8_t *src, lv_coord_t src_w, lv_coord_t src_h,
		lv_coord_t src_stride, int32_t xs_ups, int32_t ys_ups, int32_t xs_step,
		int32_t ys_step, int32_t x_end, lv_color_t *cbuf, uint8_t *abuf);
...
static void rgb565a8_no_aa(const uint8_t *src, lv_coord_t src_w,
		lv_coord_t src_h, lv_coord_t src_stride, int32_t xs_ups, int32_t ys_ups,
		int32_t xs_step, int32_t ys_step, int32_t x_end, lv_color_t *cbuf,
		uint8_t *abuf);
#endif
	...
	for (y = 0; y < dest_h; y++) {
		...
		if (draw_dsc->antialias == 0) {
			...
			case LV_IMG_CF_RGB565A8:
				rgb565a8_no_aa(src_buf, src_w, src_h, src_stride, xs_ups,
						ys_ups, xs_step_256, ys_step_256, dest_w, cbuf, abuf);
				break;
#endif
			default:
				break;
		...
		if (xs_int + x_next >= 0 && xs_int + x_next <= src_w - 1
			...
			if (has_alpha) {
				...
				if (cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
					...
					a_ver =
							*(a_tmp + ((ys_int + y_next) * src_stride) + xs_int);
				}
#endif
				...
				if (a_ver != a_base)
					a_ver = ((a_ver * ys_fract) + (a_base * (0x100 - ys_fract)))
							>> 8;
				if (a_hor != a_base)
					a_hor = ((a_hor * xs_fract) + (a_base * (0x100 - xs_fract)))
							>> 8;
