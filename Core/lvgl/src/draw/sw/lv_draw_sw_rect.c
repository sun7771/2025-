...
LV_ATTRIBUTE_FAST_MEM static void shadow_draw_corner_buf(
		const lv_area_t *coords, uint16_t *sh_buf, lv_coord_t s, lv_coord_t r);
LV_ATTRIBUTE_FAST_MEM static void shadow_blur_corner(lv_coord_t size,
		lv_coord_t sw, uint16_t *sh_ups_buf);
#endif
...
static void draw_bg(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc,
	...
	if (mask_any) {
		for (h = clipped_coords.y1; h <= clipped_coords.y2; h++) {
			...
			blend_dsc.mask_res = lv_draw_mask_apply(mask_buf, clipped_coords.x1,
					h, clipped_w);
	...
	for (h = 0; h < rout; h++) {
		 ...
		 * It saves calculating the final opa in lv_draw_sw_blend*/
		lv_memset(mask_buf, opa, clipped_w);
		blend_dsc.mask_res = lv_draw_mask_apply(mask_buf, blend_area.x1, top_y,
				clipped_w);
		if (blend_dsc.mask_res == LV_DRAW_MASK_RES_FULL_COVER)
			blend_dsc.mask_res = LV_DRAW_MASK_RES_CHANGED;
	...
	/*With gradient and/or mask draw line by line*/
		...
		for (h = bg_coords.y1 + rout; h <= h_end; h++) {
			...
			if (mask_any_center) {
				lv_memset(mask_buf, opa, clipped_w);
				blend_dsc.mask_res = lv_draw_mask_apply(mask_buf,
						clipped_coords.x1, h, clipped_w);
			}
	...
	if (src_type == LV_IMG_SRC_SYMBOL) {
		...
		if (res == LV_RES_OK) {
			...
			if (dsc->bg_img_tiled == false) {
				...
				for (; area.y1 <= coords->y2; area.y1 += header.h, area.y2 +=
						header.h) {
...
LV_ATTRIBUTE_FAST_MEM static void shadow_draw_corner_buf(
	...
	for (y = 0; y < size; y++) {
		...
		lv_draw_mask_res_t mask_res = mask_param.dsc.cb(mask_line, 0, y, size,
				&mask_param);
		if (mask_res == LV_DRAW_MASK_RES_TRANSP) {
			...
			for (i = 1; i < size; i++) {
				if (mask_line[i] == mask_line[i - 1])
					...
					sh_ups_tmp_buf[i] = (mask_line[i] << SHADOW_UPSCALE_SHIFT)
							/ sw;
			}
		}
...
void draw_border_generic(lv_draw_ctx_t *draw_ctx, const lv_area_t *outer_area,
	...
	if (mask_any) {
		...
		for (h = draw_area.y1; h <= draw_area.y2; h++) {
			...
			blend_dsc.mask_res = lv_draw_mask_apply(blend_dsc.mask_buf,
					draw_area.x1, h, draw_area_w);
			lv_draw_sw_blend(draw_ctx, &blend_dsc);
		}
	...
	if (!split_hor) {
		...
		for (h = 0; h < max_h; h++) {
			...
			lv_memset_ff(blend_dsc.mask_buf, draw_area_w);
			blend_dsc.mask_res = lv_draw_mask_apply(blend_dsc.mask_buf,
					blend_area.x1, top_y, draw_area_w);
