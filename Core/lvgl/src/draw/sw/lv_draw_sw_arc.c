...
void lv_draw_sw_arc(lv_draw_ctx_t *draw_ctx, const lv_draw_arc_dsc_t *dsc,
	...
	if (lv_area_get_width(&area_in) > 0 && lv_area_get_height(&area_in) > 0) {
		lv_draw_mask_radius_init(&mask_in_param, &area_in, LV_RADIUS_CIRCLE,
				true);
	...
	lv_draw_mask_radius_init(&mask_out_param, &area_out, LV_RADIUS_CIRCLE,
			false);
	...
	if (dsc->rounded) {
		...
		if (_lv_area_intersect(&clip_area2, clip_area_ori, &round_area)) {
			lv_draw_mask_radius_init(&mask_end_param, &round_area,
					LV_RADIUS_CIRCLE, false);
...
static void draw_quarter_0(quarter_draw_dsc_t *q) {
	...
	if (q->start_quarter == 0 && q->end_quarter == 0
		...
		bool ok = _lv_area_intersect(&quarter_area, &quarter_area,
				clip_area_ori);
		if (ok) {
			...
			quarter_area.x1 =
					q->center->x
							+ ((lv_trigo_sin(q->end_angle + 90)
									* (q->radius - q->width)) >> LV_TRIGO_SHIFT);
