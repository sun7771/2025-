 ...
 *  STATIC PROTOTYPES
 **********************/
LV_ATTRIBUTE_FAST_MEM static lv_res_t decode_and_draw(lv_draw_ctx_t *draw_ctx,
		const lv_draw_img_dsc_t *draw_dsc, const lv_area_t *coords,
		const void *src);
...
LV_ATTRIBUTE_FAST_MEM static lv_res_t decode_and_draw(lv_draw_ctx_t *draw_ctx,
	...
	_lv_img_cache_entry_t *cdsc = _lv_img_cache_open(src, draw_dsc->recolor,
			draw_dsc->frame_id);
	 ...
	 *Just draw it!*/
		...
		if (draw_dsc->angle || draw_dsc->zoom != LV_IMG_ZOOM_NONE) {
			...
			_lv_img_buf_get_transformed_area(&map_area_rot, w, h,
					draw_dsc->angle, draw_dsc->zoom, &draw_dsc->pivot);
		...
		bool union_ok;
		union_ok = _lv_area_intersect(&clip_com, draw_ctx->clip_area,
				&map_area_rot);
		...
		lv_draw_img_decoded(draw_ctx, draw_dsc, coords, cdsc->dec_dsc.img_data,
				cf);
